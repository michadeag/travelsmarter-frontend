// Generates business-hours-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-businesshours-page.js scripts/data-businesshours-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  const closurePatternLabel = CLOSURE_LABELS[c.closurePattern];
  const headline = `${c.name}'s weekend is ${c.weekendDays}. ${closurePatternLabel}.`;

  const faqs = [
    { q: `What days is the weekend in ${c.name}?`, a: `The weekend in ${c.name} is ${c.weekendDays}.` },
    { q: `What are typical business hours in ${c.name}?`, a: c.businessHours },
    { q: `Do shops close for a midday break in ${c.name}?`, a: closurePatternLabel },
    { q: `Should I plan around business hours when visiting ${c.name}?`, a: c.note },
  ];
...
<head>
    ...
    <title>Business Hours & Weekend Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="When's the weekend in ${c.name}, and what are typical business hours? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/business-hours-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Business Hours & Weekend Checker for ${c.name}">
    <meta property="og:description" content="Instant free checker: ${headline}">
    <meta property="og:url" content="https://travelsmarterapp.com/business-hours-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/business-hours-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/business-hours-checker.png">
    <meta name="twitter:title" content="Business Hours & Weekend Checker for ${c.name}">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${c.weekendDays}</span>
        <h1>Business Hours & Weekend Checker for ${c.name}</h1>
        <p>${headline}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Business Hours</button>
            </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="business-hours-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/business-hours-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'business_hours_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.closurePattern === 'sunday-closing' ? ' warn' : (data.result.closurePattern === 'siesta' || data.result.closurePattern === 'friday-prayer') ? ' caution' : '');
                document.getElementById('result-meta').textContent = \`\${data.result.businessHours} \${data.result.note}\`;
                ...
                btn.disabled = false;
                btn.textContent = 'Check Business Hours';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/business-hours-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'business_hours_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'business-hours-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-businesshours-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `business-hours-checker-${c.slug}.html`);

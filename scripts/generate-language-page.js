// Generates language-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-language-page.js scripts/data-language-countries.js
const fs = require('fs');
...
function headline(c) {
  return `${c.name}: primary language is ${c.language} — ${LEVEL_LABELS[c.englishLevel]}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const faqs = [
    { q: `What language is spoken in ${c.name}?`, a: `The primary language in ${c.name} is ${c.language}.` },
    { q: `Will I get by with English in ${c.name}?`, a: `${LEVEL_LABELS[c.englishLevel]}. ${c.note}` },
    { q: `Should I learn any phrases before visiting ${c.name}?`, a: `Learning "hello," "please," "thank you," "excuse me," and "the check please" in the local language goes a long way anywhere, including ${c.name} — locals consistently appreciate the effort.` },
    { q: `Should I download a translation app for ${c.name}?`, a: c.englishLevel === 'low' || c.englishLevel === 'moderate' ? `Yes — an offline translation app is genuinely useful in ${c.name}, especially outside major tourist areas.` : `It's rarely necessary given how widely English is understood in ${c.name}, but it doesn't hurt to have one as a backup.` },
  ];
...
<head>
    ...
    <title>Do They Speak English in ${c.name}? | TravelSmarter</title>
    <meta name="description" content="Will you get by with English in ${c.name}? Free instant checker with the local language and practical tips, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/language-checker-${c.slug}.html">
    ...
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/language-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/language-checker.png">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${c.language}</span>
        <h1>Do They Speak English in ${c.name}?</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check English Proficiency</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with practical language tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="language-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/language-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'language_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check English Proficiency';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/language-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'language_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'language-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-language-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `language-checker-${c.slug}.html`);

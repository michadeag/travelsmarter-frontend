// Generates shipping-souvenirs-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-shippingsouvenirs-page.js scripts/data-shippingsouvenirs-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = SHIPPING_LABELS[c.shippingLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const shippingLabel = SHIPPING_LABELS[c.shippingLevel];

  const faqs = [
    { q: `Can I ship souvenirs home from ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${shippingLabel}" mean?`, a: c.shippingLevel === 'reliable-postal' ? `In ${c.name}, the national postal service is reliable and commonly used for sending packages home, at a reasonable price.` : c.shippingLevel === 'courier-recommended' ? `In ${c.name}, a private courier (DHL, FedEx, UPS) is the more dependable option — national post is slower or less reliable for anything you actually care about.` : c.shippingLevel === 'limited-reliable' ? `In ${c.name}, shipping options exist but are pricier or less predictable — insure and track anything valuable.` : `In ${c.name}, formal shipping options are genuinely hard to access or unreliable — carrying souvenirs home in your luggage is the practical option.` },
    { q: `What should I do for something valuable?`, a: "Take a photo of the contents and get a tracking number regardless of destination — it makes any customs or lost-package claim far easier. For anything genuinely irreplaceable, carrying it in your luggage is the safest option everywhere." },
...
<head>
    ...
    <title>${c.name} Shipping Souvenirs Home Checker | TravelSmarter</title>
    <meta name="description" content="Can you ship souvenirs home from ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/shipping-souvenirs-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Shipping Souvenirs Home Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/shipping-souvenirs-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/shipping-souvenirs-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/shipping-souvenirs-checker.png">
    <meta name="twitter:title" content="${c.name} Shipping Souvenirs Home Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${shippingLabel}</span>
        <h1>${c.name} Shipping Souvenirs Home Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Shipping Reliability</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with shipping tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${shippingLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="shipping-souvenirs-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/shipping-souvenirs-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'shipping_souvenirs_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Shipping Reliability';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/shipping-souvenirs-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'shipping_souvenirs_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'shipping-souvenirs-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-shippingsouvenirs-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `shipping-souvenirs-checker-${c.slug}.html`);

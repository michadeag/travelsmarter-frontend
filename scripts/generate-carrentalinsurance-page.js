// Generates car-rental-insurance-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-carrentalinsurance-page.js scripts/data-carrentalinsurance-countries.js
const fs = require('fs');
...
function headline(c) {
  ...
  return `${c.name}: ${label} — typical deposit ${c.depositRange}.`;
}
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `What deposit and insurance should I expect renting a car in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${riskLabel}" mean?`, a: c.riskLevel === 'low' ? `${c.name} has a well-regulated, transparent rental market — no particularly notable surprises reported beyond standard practice.` : c.riskLevel === 'moderate' ? `Real extra costs or requirements exist in ${c.name} that are worth knowing before you sign — check the note above.` : `${c.name} has a well-documented pattern of surprise charges, aggressive upselling, or risky practices — read the note above carefully and read your rental contract in full.` },
    { q: `Should I buy the extra insurance at the counter?`, a: 'Check what your own credit card or home auto insurance already covers before buying extra coverage — you may already be covered for some or all of it. But confirm any legally mandatory local coverage separately, since that\'s not optional regardless of what else you have.' },
    { q: `What should I do before driving off?`, a: 'Photograph and video the entire car from every angle, including a timestamp — this is your best protection in any damage dispute, especially in destinations with a documented pattern of post-return charge disputes.' },
  ];
...
<head>
    ...
    <title>${c.name} Car Rental Deposit & Insurance | TravelSmarter</title>
    <meta name="description" content="What deposit and insurance should you expect renting a car in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/car-rental-insurance-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Car Rental Deposit & Insurance">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/car-rental-insurance-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/car-rental-insurance-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/car-rental-insurance-checker.png">
    <meta name="twitter:title" content="${c.name} Car Rental Deposit & Insurance">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Car Rental Deposit & Insurance</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Deposit & Insurance</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with rental car tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${riskLabel} — typical deposit ${c.depositRange}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="car-rental-insurance-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/car-rental-insurance-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'car_rental_insurance_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Deposit & Insurance';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/car-rental-insurance-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'car_rental_insurance_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'car-rental-insurance-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-carrentalinsurance-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `car-rental-insurance-checker-${c.slug}.html`);

// Generates ramadan-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-ramadan-page.js scripts/data-ramadan-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = IMPACT_LABELS[c.impactLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const impactLabel = IMPACT_LABELS[c.impactLevel];

  const faqs = [
    { q: `Does Ramadan affect a trip to ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${impactLabel}" mean?`, a: c.impactLevel === 'minimal' ? `${c.name} isn't a Muslim-majority destination, so Ramadan has no meaningful effect on the standard tourist experience.` : c.impactLevel === 'moderate' ? `${c.name} sees a real Ramadan effect, but it's mostly confined to local/non-tourist areas — international hotels and tourist zones generally continue serving visitors close to normal.` : `${c.name} sees a significant, widely-documented change to the daytime tourist experience outside international hotels — read the note above carefully before you plan your days.` },
    { q: `Is it a bad time to visit ${c.name} during Ramadan?`, a: `Not at all — evenings after sunset (iftar) are often the most festive, interesting time to be there, with special foods, markets, and extended hours. It's simply a different daytime rhythm to plan around, not a reason to avoid the trip.` },
    { q: `How do I know the exact Ramadan dates for my trip?`, a: DISCLAIMER },
  ];
...
<head>
    ...
    <title>${c.name} Ramadan Travel Impact | TravelSmarter</title>
    <meta name="description" content="Does Ramadan affect a trip to ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/ramadan-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Ramadan Travel Impact">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/ramadan-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/ramadan-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/ramadan-checker.png">
    <meta name="twitter:title" content="${c.name} Ramadan Travel Impact">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${impactLabel}</span>
        <h1>${c.name} Ramadan Travel Impact</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Ramadan Impact</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with Ramadan travel tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${impactLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="ramadan-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/ramadan-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'ramadan_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Ramadan Impact';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/ramadan-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'ramadan_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'ramadan-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-ramadan-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `ramadan-checker-${c.slug}.html`);

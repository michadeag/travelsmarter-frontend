// Generates family-travel-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-familytravel-page.js scripts/data-familytravel-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = FRIENDLINESS_LABELS[c.friendlinessLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const friendlinessLabel = FRIENDLINESS_LABELS[c.friendlinessLevel];

  const faqs = [
    { q: `Is ${c.name} family-friendly for traveling with kids?`, a: `${h} ${c.note}` },
    { q: `What does "${friendlinessLabel}" mean?`, a: c.friendlinessLevel === 'excellent' ? `${c.name} has strong infrastructure and a genuinely kid-oriented culture — few obstacles to plan around.` : c.friendlinessLevel === 'good' ? `${c.name} is solid for family travel, with few real obstacles beyond normal trip planning.` : c.friendlinessLevel === 'moderate' ? `${c.name} is doable for families, but expect real logistical friction — stairs, uneven terrain, or limited amenities.` : `${c.name} has well-documented obstacles specifically for traveling with young kids, even though the local culture is typically warm toward children.` },
    { q: `Should I bring a stroller or a baby carrier?`, a: c.friendlinessLevel === 'excellent' || c.friendlinessLevel === 'good' ? "A stroller should work fine for most of your trip, though a carrier is still handy as a backup for crowded spots." : "A carrier is likely more practical than a stroller for at least part of your trip, given the terrain and infrastructure." },
...
<head>
    ...
    <title>${c.name} Traveling with Kids Checker | TravelSmarter</title>
    <meta name="description" content="Is ${c.name} family-friendly for traveling with kids? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/family-travel-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Traveling with Kids Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/family-travel-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/family-travel-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/family-travel-checker.png">
    <meta name="twitter:title" content="${c.name} Traveling with Kids Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${friendlinessLabel}</span>
        <h1>${c.name} Traveling with Kids Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Family Friendliness</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with family travel tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${friendlinessLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="family-travel-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/family-travel-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'family_travel_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Family Friendliness';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/family-travel-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'family_travel_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'family-travel-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-familytravel-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `family-travel-checker-${c.slug}.html`);

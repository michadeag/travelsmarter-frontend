// Generates alcohol-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-alcohol-page.js scripts/data-alcohol-countries.js
const fs = require('fs');
...
function headline(c) {
  return `${c.name}: alcohol is ${STATUS_LABELS[c.status]}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const faqs = [
    { q: `Can I drink alcohol in ${c.name}?`, a: `Alcohol in ${c.name} is ${STATUS_LABELS[c.status]}. ${c.note}` },
    { q: `Where can I buy alcohol in ${c.name}?`, a: c.status === 'largely_dry' ? `Availability is very limited — check with your hotel or resort about where it's actually sold to visitors.` : `${c.note}` },
    { q: `Is there a legal drinking age in ${c.name}?`, a: `Yes — check the specific age noted above, and carry ID, since many venues check regardless of how relaxed local drinking culture seems.` },
    { q: `Are there times when alcohol sales are banned in ${c.name}?`, a: `Possibly — some countries ban retail alcohol sales on election days or during specific religious periods like Ramadan, even where alcohol is otherwise freely available. Check close to your travel dates if this could affect you.` },
  ];
...
<head>
    ...
    <title>Alcohol Laws in ${c.name} | TravelSmarter</title>
    <meta name="description" content="Is alcohol freely available in ${c.name}? Free instant checker with purchase rules and special restrictions, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/alcohol-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Alcohol Laws in ${c.name}">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/alcohol-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/alcohol-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/alcohol-checker.png">
    <meta name="twitter:title" content="Alcohol Laws in ${c.name}">
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Alcohol Laws</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with tips for navigating local rules in ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="alcohol-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/alcohol-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'alcohol_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Alcohol Laws';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/alcohol-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'alcohol_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'alcohol-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-alcohol-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `alcohol-checker-${c.slug}.html`);

// Generates resort-fee-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-resortfee-page.js scripts/data-resortfee-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  const prevalenceLabel = PREVALENCE_LABELS[c.prevalence];
  const headline = c.prevalence === 'none'
    ? `${c.name}: hidden resort/destination fees are ${prevalenceLabel}.`
    : `${c.name}: hidden resort/destination fees are ${prevalenceLabel}. Typical range where they apply: ${c.typicalFeeRange}.`;

  const faqs = [
    { q: `Do hotels in ${c.name} charge hidden resort fees?`, a: `${c.name}: hidden resort/destination fees are ${prevalenceLabel} ${c.note}` },
    { q: c.prevalence === 'none' ? `Are there any extra charges I should watch for in ${c.name}?` : `How much are resort fees typically in ${c.name}?`, a: c.prevalence === 'none' ? `No widespread hotel resort fee practice exists here — always check the final checkout total against the advertised rate regardless.` : `Where they apply, resort fees typically run ${c.typicalFeeRange}.` },
    { q: `Is a resort fee the same as a tourist tax in ${c.name}?`, a: `No — a resort fee is charged by the hotel itself, while a tourist tax is a separate government charge. Both can appear on the same bill.` },
    { q: `How can I avoid surprise resort fees when booking in ${c.name}?`, a: `Search the hotel's booking page for "resort fee," "destination fee," or "amenity fee" before booking, and compare the true total cost (room rate plus mandatory fees), not just the headline nightly rate.` },
  ];
...
<head>
    ...
    <title>Resort Fee Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="Do hotels in ${c.name} charge hidden resort fees? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/resort-fee-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Resort Fee Checker for ${c.name}">
    <meta property="og:description" content="Instant free checker: ${headline}">
    <meta property="og:url" content="https://travelsmarterapp.com/resort-fee-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/resort-fee-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/resort-fee-checker.png">
    <meta name="twitter:title" content="Resort Fee Checker for ${c.name}">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${c.prevalence} prevalence</span>
        <h1>Resort Fee Checker for ${c.name}</h1>
        <p>${headline}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Resort Fees</button>
            </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="resort-fee-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/resort-fee-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'resort_fee_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.prevalence === 'high' ? ' warn' : data.result.prevalence === 'moderate' ? ' caution' : '');
                ...
                btn.disabled = false;
                btn.textContent = 'Check Resort Fees';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/resort-fee-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'resort_fee_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'resort-fee-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-resortfee-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `resort-fee-checker-${c.slug}.html`);

// Generates restaurant-pace-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-restaurantpace-page.js scripts/data-restaurantpace-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = PACE_LABELS[c.paceLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const paceLabel = PACE_LABELS[c.paceLevel];

  const faqs = [
    { q: `Will the check come automatically in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${paceLabel}" mean?`, a: c.paceLevel === 'leisurely' ? `In ${c.name}, meals are unhurried by design — the check is essentially never brought unprompted, so you need to actively ask for it.` : c.paceLevel === 'relaxed' ? `In ${c.name}, the pace is noticeably slower than fast-food culture, though the check comes reasonably promptly once you ask.` : c.paceLevel === 'moderate' ? `In ${c.name}, the check typically comes once staff notice you've finished, without a long wait either way.` : `In ${c.name}, fast table turnover is the norm, and the check is often brought proactively without you needing to ask.` },
    { q: `Does this apply to fine dining too?`, a: 'Not exactly — high-end fine dining tends to run more leisurely everywhere, and fast-casual or counter-service spots run brisker everywhere, regardless of the destination\'s general pace.' },
...
<head>
    ...
    <title>${c.name} Restaurant Pace Checker | TravelSmarter</title>
    <meta name="description" content="Will the check come automatically in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/restaurant-pace-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Restaurant Pace Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/restaurant-pace-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/restaurant-pace-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/restaurant-pace-checker.png">
    <meta name="twitter:title" content="${c.name} Restaurant Pace Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${paceLabel}</span>
        <h1>${c.name} Restaurant Pace Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Restaurant Pace</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with dining pace tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${paceLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="restaurant-pace-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/restaurant-pace-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'restaurant_pace_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Restaurant Pace';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/restaurant-pace-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'restaurant_pace_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'restaurant-pace-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-restaurantpace-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `restaurant-pace-checker-${c.slug}.html`);

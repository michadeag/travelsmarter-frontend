// Generates gift-giving-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-giftgiving-page.js scripts/data-giftgiving-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = GIFT_LABELS[c.giftCultureLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const giftLabel = GIFT_LABELS[c.giftCultureLevel];

  const faqs = [
    { q: `What should I bring as a gift in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${giftLabel}" mean?`, a: c.giftCultureLevel === 'highly-ritualized' ? `In ${c.name}, gift-giving follows real, specific etiquette rules — how a gift is given, wrapped, and received all matter, not just what it is.` : c.giftCultureLevel === 'valued-with-taboos' ? `In ${c.name}, gifts are genuinely appreciated, and there are a few specific things worth avoiding.` : c.giftCultureLevel === 'casual-few-rules' ? `In ${c.name}, a gift is a nice, appreciated gesture, but there's no strict etiquette or major taboo to worry about.` : `In ${c.name}, gift-giving for casual visits isn't a strong cultural expectation — it's a nice gesture, not a requirement.` },
    { q: `Does this apply to business gifts too?`, a: 'Not necessarily — business gift-giving often has its own, stricter rules (or outright company policies against it) separate from social/home-visit customs. Research your specific counterpart\'s context before bringing a business gift.' },
...
<head>
    ...
    <title>${c.name} Gift-Giving Etiquette Checker | TravelSmarter</title>
    <meta name="description" content="What should you bring as a gift in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/gift-giving-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Gift-Giving Etiquette Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/gift-giving-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/gift-giving-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/gift-giving-checker.png">
    <meta name="twitter:title" content="${c.name} Gift-Giving Etiquette Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${giftLabel}</span>
        <h1>${c.name} Gift-Giving Etiquette Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Gift-Giving Norms</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with gift-giving tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${giftLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="gift-giving-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/gift-giving-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'gift_giving_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Gift-Giving Norms';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/gift-giving-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'gift_giving_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'gift-giving-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-giftgiving-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `gift-giving-checker-${c.slug}.html`);

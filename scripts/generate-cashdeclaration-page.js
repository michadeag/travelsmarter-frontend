// Generates cash-declaration-checker-{slug}.html pages, one per
// country, synthesizing FAQ text from the same fields the backend
// calculator uses so content stays consistent with the live API.
// Run: node scripts/generate-cashdeclaration-page.js scripts/data-cashdeclaration-countries.js
const fs = require('fs');
...
function headline(c) {
  ...
  return `${c.name}: ${label} — declare cash over ${c.limit}.`;
}
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `How much cash can I bring into or out of ${c.name} without declaring it?`, a: `${h} ${c.note}` },
    { q: `What does "${riskLabel}" mean?`, a: c.riskLevel === 'low' ? `${c.name} uses a standard, straightforward threshold in line with the common global benchmark — easy to stay compliant with if you're aware of the number.` : c.riskLevel === 'moderate' ? `${c.name}'s threshold is notably lower than the common $10,000 global benchmark, or has a documented local wrinkle — check the note above so you don't trip it by accident.` : `${c.name} runs a genuinely strict or complex regime — currency controls, a closed local currency, or asymmetric domestic/foreign-currency rules. Read the note above carefully before you travel with cash.` },
    { q: `What happens if I don't declare cash over the limit in ${c.name}?`, a: `Undeclared cash over the threshold can be seized outright at customs, even if the money is completely legitimate — declaring late or claiming you forgot isn't a valid defense in most countries. Some countries also apply fines on top of seizure.` },
    { q: `Does this limit apply both entering and leaving ${c.name}?`, a: `In most countries, yes — the same threshold typically applies in both directions, and it usually covers cash, traveler's cheques, and similar bearer instruments combined, not just banknotes. Always verify the current rule on the destination's official customs website before you fly.` },
  ];
...
<head>
    ...
    <title>${c.name} Cash Declaration Limit | TravelSmarter</title>
    <meta name="description" content="How much cash can you bring into or out of ${c.name} without declaring it? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/cash-declaration-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Cash Declaration Limit">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/cash-declaration-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/cash-declaration-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/cash-declaration-checker.png">
    <meta name="twitter:title" content="${c.name} Cash Declaration Limit">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Cash Declaration Limit</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Cash Limit</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with cash declaration tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${riskLabel}: declare cash over ${c.limit}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="cash-declaration-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/cash-declaration-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'cash_declaration_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Cash Limit';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/cash-declaration-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'cash_declaration_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'cash-declaration-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-cashdeclaration-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `cash-declaration-checker-${c.slug}.html`);

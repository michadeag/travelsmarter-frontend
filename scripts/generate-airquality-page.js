// Generates air-quality-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-airquality-page.js scripts/data-airquality-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = AQI_LABELS[c.aqiLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const aqiLabel = AQI_LABELS[c.aqiLevel];

  const faqs = [
    { q: `Is the air quality safe in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${aqiLabel}" mean?`, a: c.aqiLevel === 'good' ? `${c.name} consistently has clean air with no significant pollution sources.` : c.aqiLevel === 'moderate' ? `${c.name} has real, often seasonal or localized pollution worth knowing about, especially if you have asthma or allergies.` : `${c.name} has well-documented, frequently unhealthy air quality — sensitive groups should take real precautions and check a real-time index before outdoor activity.` },
    { q: `Should I bring a mask?`, a: c.aqiLevel === 'unhealthy' ? "Yes — an N95 or KN95 mask is a compact, effective precaution for the days when the local index is elevated, especially if you have a respiratory condition." : "Not typically needed for most travelers, but if you have asthma or allergies, it's a cheap, compact backup worth packing regardless of destination." },
...
<head>
    ...
    <title>${c.name} Air Quality Checker | TravelSmarter</title>
    <meta name="description" content="Is the air quality safe in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/air-quality-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Air Quality Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/air-quality-checker-${c.slug}.html">
    <meta property="og:image" content="https://travelsmarterapp.com/og-images/air-quality-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://travelsmarterapp.com/og-images/air-quality-checker.png">
    <meta name="twitter:title" content="${c.name} Air Quality Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${aqiLabel}</span>
        <h1>${c.name} Air Quality Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Air Quality</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with air quality tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${aqiLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="air-quality-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/air-quality-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'air_quality_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Air Quality';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/air-quality-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'air_quality_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'air-quality-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-airquality-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `air-quality-checker-${c.slug}.html`);

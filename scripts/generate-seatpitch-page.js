// Generates seat-pitch-checker-{slug}.html pages, one per airline,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-seatpitch-page.js scripts/data-seatpitch-airlines.js
const fs = require('fs');
const path = require('path');

function headline(a) {
  return a.premiumName
    ? `${a.name}: standard pitch is ${a.standardPitchIn}", ${a.premiumName} offers ${a.premiumPitchIn}".`
    : `${a.name}: standard pitch is ${a.standardPitchIn}" — no separate premium-legroom cabin.`;
}
...
function render(a, allAirlines) {
  const h = headline(a);
  const faqs = [
    { q: `What is the seat pitch on ${a.name}?`, a: `Standard economy seat pitch on ${a.name} is ${a.standardPitchIn} inches.` },
    { q: a.premiumName ? `Is ${a.premiumName} worth it on ${a.name}?` : `Does ${a.name} offer extra-legroom seating?`, a: `${a.note}` },
    { q: `Does seat pitch vary by aircraft on ${a.name}?`, a: `Yes — pitch can vary by aircraft type and specific route even within the same fare class, so always check a seat-map site for your exact flight number before booking.` },
    { q: `What's the cheapest way to get extra legroom on ${a.name}?`, a: `Exit rows and bulkhead seats often have similar or better legroom than the paid premium option, sometimes for a lower seat-selection fee — check the seat map before paying for the named premium product.` },
  ];
...
<head>
    ...
    <title>${a.name} Seat Pitch & Legroom Guide | TravelSmarter</title>
    <meta name="description" content="How much legroom will you get on ${a.name}? Free instant checker with standard and premium seat pitch, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/seat-pitch-checker-${a.slug}.html">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${a.name} · ${a.standardPitchIn}" standard</span>
        <h1>${a.name} Seat Pitch & Legroom Guide</h1>
        <p>${h}</p>
    </div>
...
${airlineOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Seat Pitch</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with tips for getting the most legroom on ${a.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="seat-pitch-checker.html">Check a different airline →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/seat-pitch-checker/calculate\`, {
                    method: 'POST',
                ...
                gtag('event', 'tool_calculate', { tool: 'seat_pitch_checker_${a.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Seat Pitch';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/seat-pitch-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'seat_pitch_checker_${a.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a2 = document.createElement('a');
                a2.href = url;
                a2.download = 'seat-pitch-checker-${a.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-seatpitch-page.js <data-file.js>');
  process.exit(1);
}
...
airlines.forEach(a => {
  ...
  const outPath = path.join(outDir, `seat-pitch-checker-${a.slug}.html`);

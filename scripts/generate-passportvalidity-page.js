// Generates passport-validity-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-passportvalidity-page.js scripts/data-passportvalidity-countries.js
const fs = require('fs');
...
const REQUIREMENT_SHORT = {
  'six-months': '6-month rule',
  'three-months': '3-month rule (Schengen)',
  'stay-duration': 'stay-duration only',
};

function headline(c) {
  return `${c.name} ${REQUIREMENT_LABELS[c.requirement]}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const faqs = [
    { q: `How much passport validity does ${c.name} require?`, a: h },
    { q: `What happens if my passport doesn't meet ${c.name}'s validity rule?`, a: `Airlines routinely deny boarding at check-in if your passport doesn't meet the destination's stated validity rule, even before you reach immigration — it's worth checking well before you book, not just before you fly.` },
    { q: `Is ${c.name}'s passport rule the same as its visa rule?`, a: `No — passport validity and visa requirements are separate checks. ${c.note}` },
    { q: `Should I renew my passport before booking a trip to ${c.name}?`, a: `If your passport's expiry date is close to the cutoff this checker calculates, renew before booking — passport offices often take longer than travelers expect, especially in peak season.` },
  ];
...
<head>
    ...
    <title>Passport Validity Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="How much passport validity does ${c.name} require? Free instant checker — enter your passport expiry date and get the exact latest safe travel date, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/passport-validity-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Passport Validity Checker for ${c.name}">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/passport-validity-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/passport-validity-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/passport-validity-checker.png">
    <meta name="twitter:title" content="Passport Validity Checker for ${c.name}">
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .result-headline.urgent { background:#fee2e2; color:#991b1b; }
        .result-headline.soon { background:#fffbeb; color:#92400e; }
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${REQUIREMENT_SHORT[c.requirement]}</span>
        <h1>Passport Validity Checker for ${c.name}</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <label for="expiryDate">Your passport's expiry date</label>
                <input type="date" id="expiryDate">

                <div id="calc-alert" class="alert alert-error"></div>
                <button class="btn" id="calc-btn" onclick="calculate()">Check Passport Validity</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with your exact safe-travel cutoff and renewal tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="passport-validity-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            if (!expiryDate) {
                alertEl.textContent = "Please enter your passport's expiry date.";
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/passport-validity-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ country, expiryDate }),
                });
                const data = await res.json();
                if (!data.success) throw new Error(data.error || 'Something went wrong');

                lastResult = data.result;
                gtag('event', 'tool_calculate', { tool: 'passport_validity_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.urgency === 'expired' ? ' urgent' : data.result.urgency === 'soon' ? ' soon' : '');
                ...
                btn.disabled = false;
                btn.textContent = 'Check Passport Validity';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/passport-validity-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'passport_validity_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'passport-validity-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-passportvalidity-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `passport-validity-checker-${c.slug}.html`);

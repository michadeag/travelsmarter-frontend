// Generates altitude-sickness-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-altitude-page.js scripts/data-altitude-countries.js
const fs = require('fs');
const path = require('path');

const RISK_LABELS = {
  none: 'None — Essentially Sea Level',
  low: 'Low — One High-Elevation Spot, Most Visitors Won\'t Go There',
  moderate: 'Moderate — A Popular Destination Sits High Enough to Matter',
  high: 'High — A Major, Frequently Visited Destination Carries Real Risk',
};

const DISCLAIMER = "This reflects the elevation of popular tourist destinations, not a medical assessment — individual susceptibility to altitude sickness varies a lot and isn't predicted by fitness level. If you're headed somewhere moderate or high risk, ascend gradually where possible, stay hydrated, avoid alcohol on arrival day, and talk to a travel clinic about preventive medication (like acetazolamide) if you have a history of altitude symptoms.";

function headline(c) {
  const label = RISK_LABELS[c.riskLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const riskLabel = RISK_LABELS[c.riskLevel];

  const faqs = [
    { q: `Do I need to worry about altitude sickness in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${riskLabel}" mean?`, a: c.riskLevel === 'none' ? `${c.name}'s tourist destinations sit at essentially sea level — altitude sickness isn't a concern.` : c.riskLevel === 'low' ? `${c.name} has a specific high-elevation spot, but most visitors won't go there — it's only a concern if your itinerary includes it.` : c.riskLevel === 'moderate' ? `${c.name} has a popular, commonly visited destination that sits high enough for mild symptoms to be common.` : `${c.name} has a major, frequently visited destination where real altitude sickness — not just mild discomfort — is a well-documented risk.` },
    { q: `Does being physically fit protect against altitude sickness?`, a: "No — this is one of the most common misconceptions. Fitness level doesn't predict who gets hit; even very fit travelers can experience symptoms, while less fit travelers sometimes feel fine. Genetics and how quickly you ascend matter far more." },
...
<head>
    ...
    <title>${c.name} Altitude Sickness Checker | TravelSmarter</title>
    <meta name="description" content="Do you need to worry about altitude sickness in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/altitude-sickness-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Altitude Sickness Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/altitude-sickness-checker-${c.slug}.html">
    <meta property="og:image" content="https://travelsmarterapp.com/og-images/altitude-sickness-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://travelsmarterapp.com/og-images/altitude-sickness-checker.png">
    <meta name="twitter:title" content="${c.name} Altitude Sickness Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${riskLabel}</span>
        <h1>${c.name} Altitude Sickness Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Altitude Sickness Risk</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with altitude sickness tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${riskLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="altitude-sickness-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/altitude-sickness-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'altitude_sickness_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Altitude Sickness Risk';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/altitude-sickness-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'altitude_sickness_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'altitude-sickness-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-altitude-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `altitude-sickness-checker-${c.slug}.html`);

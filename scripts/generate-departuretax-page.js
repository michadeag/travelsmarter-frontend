// Generates departure-tax-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-departuretax-page.js scripts/data-departuretax-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Does ${c.name} charge a separate departure tax?`, a: `${c.name} ${label}. ${c.note}` },
    { q: `Do I need cash for departure from ${c.name}?`, a: c.status === 'verify' ? `It's worth keeping a small amount of local currency on hand just in case, given ${c.name}'s track record — but confirm current practice with your airline before you fly.` : `No — ${c.name}'s departure-related charges are already included in your ticket price, so you don't need to budget separately.` },
    { q: `Is the departure tax the same as a tourist tax in ${c.name}?`, a: `No — a departure tax is charged when leaving the country by air, usually via the ticket price, while a tourist/city tax (where it exists) is typically charged per night at accommodation. They're separate fees.` },
    { q: `Does this apply to smaller regional airports in ${c.name} too?`, a: `Practices can differ from the main international hub at smaller regional airports — if you're departing from one, it's worth a quick specific check.` },
  ];
...
<head>
    ...
    <title>Departure Tax Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="Does ${c.name} charge a separate airport departure tax? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/departure-tax-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Departure Tax Checker for ${c.name}">
    <meta property="og:description" content="Instant free checker: ${c.name} ${label}.">
    <meta property="og:url" content="https://travelsmarterapp.com/departure-tax-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/departure-tax-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/departure-tax-checker.png">
    <meta name="twitter:title" content="Departure Tax Checker for ${c.name}">
    <meta name="twitter:description" content="Instant free checker: ${c.name} ${label}.">
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .result-headline.warn { background:#fffbeb; color:#92400e; }
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${c.status}</span>
        <h1>Departure Tax Checker for ${c.name}</h1>
        <p>${c.name} ${label}.</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Departure Tax</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with departure tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="departure-tax-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/departure-tax-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'departure_tax_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.status === 'verify' ? ' warn' : '');
                ...
                btn.disabled = false;
                btn.textContent = 'Check Departure Tax';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/departure-tax-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'departure_tax_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'departure-tax-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-departuretax-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `departure-tax-checker-${c.slug}.html`);

// Generates greeting-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-greeting-page.js scripts/data-greeting-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = GREETING_LABELS[c.greetingStyle];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const greetingLabel = GREETING_LABELS[c.greetingStyle];

  const faqs = [
    { q: `How do people greet each other in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${greetingLabel}" mean?`, a: c.greetingStyle === 'handshake' ? `In ${c.name}, a handshake is the standard, expected greeting in both business and social settings.` : c.greetingStyle === 'cheek-kiss' ? `In ${c.name}, cheek kissing (or an air-kiss) between acquaintances is normal and expected in social settings, alongside or instead of a handshake.` : c.greetingStyle === 'bow' ? `In ${c.name}, a bow or non-contact gesture is the standard respectful greeting, and physical contact with people you've just met is generally avoided.` : `In ${c.name}, a light handshake or verbal greeting is fine, but personal space is valued and closer contact is reserved for close relationships.` },
    { q: `What if I'm not sure what to do?`, a: "A light handshake or friendly nod is a safe default almost anywhere in the world. Let the other person initiate anything closer, like a hug or cheek kiss — following their lead avoids any awkwardness." },
...
<head>
    ...
    <title>${c.name} Greeting & Personal Space Checker | TravelSmarter</title>
    <meta name="description" content="How do people greet each other in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/greeting-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Greeting & Personal Space Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/greeting-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/greeting-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/greeting-checker.png">
    <meta name="twitter:title" content="${c.name} Greeting & Personal Space Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${greetingLabel}</span>
        <h1>${c.name} Greeting & Personal Space Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Greeting Norms</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with greeting tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${greetingLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="greeting-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/greeting-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'greeting_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Greeting Norms';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/greeting-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'greeting_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'greeting-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-greeting-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `greeting-checker-${c.slug}.html`);

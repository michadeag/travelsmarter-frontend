// Generates tourist-tax-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-touristtax-page.js scripts/data-touristtax-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  const label = LEVEL_LABELS[c.level];
  const faqs = [
    { q: `Does ${c.name} charge a tourist tax?`, a: `${c.name} ${label}. ${c.note}` },
    { q: `How much is the tourist tax in ${c.name}?`, a: c.amountNote || `${c.name} doesn't have a widespread tourist tax to budget for, though local site entry fees may still apply at specific attractions.` },
    { q: `How do I pay the tourist tax in ${c.name}?`, a: c.paymentNote || `Since ${c.name} doesn't have a widespread tourist tax, there's typically nothing extra to pay at check-in beyond your booked rate.` },
    { q: `Is the tourist tax included in the price shown when I book in ${c.name}?`, a: `Almost never — tourist and accommodation taxes are typically excluded from the headline price shown by booking sites and added separately at check-in or check-out, so it's worth budgeting a small buffer.` },
  ];
...
<head>
    ...
    <title>Tourist Tax Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="Does ${c.name} charge a tourist tax or city tax? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/tourist-tax-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Tourist Tax Checker for ${c.name}">
    <meta property="og:description" content="Instant free checker: ${c.name} ${label}.">
    <meta property="og:url" content="https://travelsmarterapp.com/tourist-tax-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/tourist-tax-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/tourist-tax-checker.png">
    <meta name="twitter:title" content="Tourist Tax Checker for ${c.name}">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${c.level}</span>
        <h1>Tourist Tax Checker for ${c.name}</h1>
        <p>${c.name} ${label}.</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Tourist Tax</button>
            </div>

            <div id="result">
                <div class="result-headline" id="result-headline"></div>
                <div class="result-meta" id="result-amount"></div>
                <div class="result-meta" id="result-payment"></div>

                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with budgeting tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="tourist-tax-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/tourist-tax-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'tourist_tax_checker_${c.slug}' });

                document.getElementById('result-headline').textContent = data.result.headline;
                document.getElementById('result-amount').textContent = data.result.amountNote || '';
                document.getElementById('result-payment').textContent = data.result.paymentNote || '';
                ...
                btn.disabled = false;
                btn.textContent = 'Check Tourist Tax';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/tourist-tax-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'tourist_tax_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'tourist-tax-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-touristtax-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `tourist-tax-checker-${c.slug}.html`);

// Generates holiday-season-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-holidayseason-page.js scripts/data-holidayseason-countries.js
const fs = require('fs');
...
function headline(c) {
  return `${c.name}: ${c.season} — ${IMPACT_LABELS[c.impact]}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const impactLabel = IMPACT_LABELS[c.impact];

  const faqs = [
    { q: `Is there a disruptive holiday season in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${impactLabel}" mean?`, a: c.impact === 'high' ? `Expect real disruption — many businesses closed, transport and hotels booked out well in advance, and significant price surges.` : c.impact === 'moderate' ? `Some disruption is likely — higher prices and crowds, with a mix of closures depending on the specific business.` : `Little to no disruption expected from a single dominant season — plan around typical peak-season pricing instead.` },
    { q: `Are the exact dates given here?`, a: `Not exact dates — many of these holidays (Ramadan, Lunar New Year, Easter-linked observances) shift every year on lunar or moveable calendars. Search the specific year of your trip to confirm exact dates.` },
    { q: `Should I avoid visiting ${c.name} during this season?`, a: `Not necessarily — major tourist attractions and international hotels usually stay open even during high-disruption seasons, and some (like Carnival or Songkran) are genuinely worth experiencing if you plan around them. Just book well ahead and expect higher prices.` },
  ];
...
<head>
    ...
    <title>${c.name} Holiday Season Guide | TravelSmarter</title>
    <meta name="description" content="Is there a disruptive holiday season in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/holiday-season-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Holiday Season Guide</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Holiday Season</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with trip-timing tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${c.season} — ${impactLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="holiday-season-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/holiday-season-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'holiday_season_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Holiday Season';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/holiday-season-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'holiday_season_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'holiday-season-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-holidayseason-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `holiday-season-checker-${c.slug}.html`);

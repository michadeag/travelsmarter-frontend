// Generates minor-consent-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-minorconsent-page.js scripts/data-minorconsent-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = ENFORCEMENT_LABELS[c.enforcementLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const enforcementLabel = ENFORCEMENT_LABELS[c.enforcementLevel];

  const faqs = [
    { q: `Do I need a consent letter to bring my child to ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${enforcementLabel}" mean?`, a: c.enforcementLevel === 'standard' ? `${c.name} doesn't routinely check for this at the border, but a notarized letter is still universally recommended regardless of destination — individual officers and airlines retain discretion to ask.` : c.enforcementLevel === 'moderate' ? `${c.name}'s immigration officials commonly ask for documented consent at their discretion, especially for minors traveling with only one parent or a non-parent — carrying a notarized letter avoids delays.` : `${c.name} is one of the most well-documented, consistently enforced examples globally — read the note above carefully and verify the current exact requirement before you travel.` },
    { q: `What should a good consent letter include?`, a: "The absent parent's or guardian's full name, signature, and contact information; the traveling parent's or companion's name and relationship to the child; the specific travel dates and destination(s); and notarization — a signature alone usually isn't enough." },
    { q: `Does this only matter on arrival?`, a: DISCLAIMER },
  ];
...
<head>
    ...
    <title>${c.name} Minor Travel Consent Letter | TravelSmarter</title>
    <meta name="description" content="Do you need a notarized consent letter to bring your child to ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/minor-consent-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Minor Travel Consent Letter">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/minor-consent-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/minor-consent-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/minor-consent-checker.png">
    <meta name="twitter:title" content="${c.name} Minor Travel Consent Letter">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${enforcementLabel}</span>
        <h1>${c.name} Minor Travel Consent Letter</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Consent Letter Requirement</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with consent letter tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${enforcementLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="minor-consent-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/minor-consent-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'minor_consent_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Consent Letter Requirement';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/minor-consent-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'minor_consent_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'minor-consent-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-minorconsent-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `minor-consent-checker-${c.slug}.html`);

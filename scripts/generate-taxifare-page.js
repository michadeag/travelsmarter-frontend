// Generates taxi-fare-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-taxifare-page.js scripts/data-taxifare-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = FARE_LABELS[c.fareLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const fareLabel = FARE_LABELS[c.fareLevel];

  const faqs = [
    { q: `Do taxis use meters in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${fareLabel}" mean?`, a: c.fareLevel === 'metered' ? `In ${c.name}, meters are standard, legally required, and reliably used — insist on one being switched on.` : c.fareLevel === 'negotiate' ? `In ${c.name}, fares are typically agreed upfront before the ride, since metering isn't standard or reliably enforced.` : `In ${c.name}, a ride-hailing app is the practical default for visitors, sidestepping street-taxi fare uncertainty altogether.` },
    { q: `What if a driver refuses to use the meter?`, a: 'That\'s a signal to either insist firmly, get out and find another cab, or switch to a ride-hailing app if one is available — don\'t assume you have to accept whatever is offered.' },
...
<head>
    ...
    <title>${c.name} Taxi Fare Checker | TravelSmarter</title>
    <meta name="description" content="Do taxis use meters in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/taxi-fare-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Taxi Fare Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/taxi-fare-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/taxi-fare-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/taxi-fare-checker.png">
    <meta name="twitter:title" content="${c.name} Taxi Fare Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${fareLabel}</span>
        <h1>${c.name} Taxi Fare Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Taxi Fare Norms</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with taxi tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${fareLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="taxi-fare-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/taxi-fare-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'taxi_fare_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Taxi Fare Norms';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/taxi-fare-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'taxi_fare_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'taxi-fare-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-taxifare-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `taxi-fare-checker-${c.slug}.html`);

// Generates bargaining-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-bargaining-page.js scripts/data-bargaining-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = HAGGLE_LABELS[c.haggleLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const haggleLabel = HAGGLE_LABELS[c.haggleLevel];

  const faqs = [
    { q: `Should I haggle in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${haggleLabel}" mean?`, a: c.haggleLevel === 'expected' ? `Haggling is a normal, expected part of shopping in ${c.name} — sellers typically quote an inflated opening price anticipating negotiation.` : c.haggleLevel === 'sometimes' ? `In ${c.name}, haggling works in informal markets and with independent vendors, but not in stores, malls, or supermarkets, where prices are fixed.` : c.haggleLevel === 'rare' ? `Most shopping in ${c.name} is fixed-price — haggling is only really possible at flea markets or with independent craft vendors.` : `Fixed prices are the strong norm in ${c.name} — attempting to negotiate in a regular shop would be unusual.` },
    { q: `Where does haggling actually work in ${c.name}?`, a: c.haggleLevel === 'expected' || c.haggleLevel === 'sometimes' ? 'Street markets, independent vendor stalls, and tourist-facing souvenir shops — never at supermarkets, malls, chain stores, or anywhere with posted, printed prices.' : 'Flea markets or informal craft stalls, if you find any — regular retail is fixed-price and negotiating there would be unusual.' },
...
<head>
    ...
    <title>${c.name} Bargaining & Haggling Checker | TravelSmarter</title>
    <meta name="description" content="Should you haggle in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/bargaining-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Bargaining & Haggling Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/bargaining-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/bargaining-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/bargaining-checker.png">
    <meta name="twitter:title" content="${c.name} Bargaining & Haggling Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${haggleLabel}</span>
        <h1>${c.name} Bargaining & Haggling Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Bargaining Norms</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with bargaining tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${haggleLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="bargaining-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/bargaining-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'bargaining_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Bargaining Norms';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/bargaining-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'bargaining_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'bargaining-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-bargaining-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `bargaining-checker-${c.slug}.html`);

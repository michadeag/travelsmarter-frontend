// Generates wildlife-safety-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-wildlifesafety-page.js scripts/data-wildlifesafety-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `What are the wildlife safety risks in ${c.name}?`, a: `${c.name}'s wildlife hazard level is ${label}. ${c.note}` },
    { q: `What should I do if bitten or stung in ${c.name}?`, a: 'Stay calm, keep the affected limb still and below heart level if possible, and get to medical care immediately — never attempt to suck out venom or apply a tourniquet, both are outdated and can cause more harm.' },
    { q: `Is it safe to hike or camp in ${c.name}?`, a: c.level === 'high' ? `Take extra precautions — wear closed-toe shoes on trails, check bedding and shoes in rural accommodation, and book guided tours for wildlife-viewing activities.` : `Standard outdoor precautions apply, but dangerous wildlife is a low practical concern for typical hiking and camping in ${c.name}.` },
    { q: `Should I book a guided tour for wildlife areas in ${c.name}?`, a: `Yes, especially for safari, jungle, or rainforest activities — guides know current local hazards and how to respond to them, and significantly reduce risk compared to exploring independently.` },
  ];
...
<head>
    ...
    <title>Wildlife Safety Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="What are the wildlife and venomous animal risks in ${c.name}? Free instant checker with safety tips, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/wildlife-safety-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>Wildlife Safety Checker for ${c.name}</h1>
        <p>${c.name}'s wildlife hazard level is ${label}.</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Wildlife Safety</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with wildlife safety tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="wildlife-safety-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/wildlife-safety-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'wildlife_safety_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.level === 'high' ? ' warn' : '');
                ...
                btn.disabled = false;
                btn.textContent = 'Check Wildlife Safety';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/wildlife-safety-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'wildlife_safety_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'wildlife-safety-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-wildlifesafety-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `wildlife-safety-checker-${c.slug}.html`);

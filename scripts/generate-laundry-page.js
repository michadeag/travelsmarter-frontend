// Generates laundry-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-laundry-page.js scripts/data-laundry-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = LAUNDRY_LABELS[c.laundryLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const laundryLabel = LAUNDRY_LABELS[c.laundryLevel];

  const faqs = [
    { q: `Where can I get laundry done in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${laundryLabel}" mean?`, a: c.laundryLevel === 'cheap-and-common' ? `In ${c.name}, affordable drop-off wash-and-fold laundry services are widely available and cheap, typically priced by the kilo.` : c.laundryLevel === 'self-service-common' ? `In ${c.name}, self-service coin laundromats are widespread and easy to find.` : c.laundryLevel === 'limited' ? `In ${c.name}, laundry options exist but are less convenient or pricier than a dedicated laundromat — small local services or dry cleaners are the more typical option.` : `In ${c.name}, outside laundry access is genuinely scarce — hotel or resort laundry, often expensive, is the practical option.` },
    { q: `What if my hotel doesn't offer laundry?`, a: 'Ask directly, even at a budget guesthouse — many have an informal in-house arrangement that never shows up in a search. Laundry pickup apps also increasingly cover major cities worldwide.' },
...
<head>
    ...
    <title>${c.name} Laundry Access Checker | TravelSmarter</title>
    <meta name="description" content="Where can you get laundry done in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/laundry-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Laundry Access Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/laundry-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/laundry-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/laundry-checker.png">
    <meta name="twitter:title" content="${c.name} Laundry Access Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${laundryLabel}</span>
        <h1>${c.name} Laundry Access Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Laundry Access</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with laundry tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${laundryLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="laundry-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/laundry-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'laundry_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Laundry Access';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/laundry-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'laundry_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'laundry-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-laundry-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `laundry-checker-${c.slug}.html`);

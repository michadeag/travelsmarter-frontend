// Generates cashless-payment-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-cashlesspayment-page.js scripts/data-cashlesspayment-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  const cardAcceptanceLabel = CARD_LABELS[c.cardAcceptance];
  const headline = `${c.name}'s card acceptance is ${cardAcceptanceLabel}. Keep cash on hand for ${c.cashSituations}.`;

  const faqs = [
    { q: `Can I pay by card everywhere in ${c.name}?`, a: `${c.name}'s card acceptance is ${cardAcceptanceLabel}. ${c.note}` },
    { q: `Do I need to carry cash in ${c.name}?`, a: `Yes, mainly for ${c.cashSituations}.` },
    { q: `Is contactless/tap-to-pay common in ${c.name}?`, a: c.cardAcceptance === 'high' ? `Yes — contactless is widely used and often the default payment method in ${c.name}.` : c.cardAcceptance === 'moderate' ? `Contactless works at most established businesses in ${c.name}, though smaller vendors may only take cash.` : `Contactless is limited in ${c.name} — plan on cash or local mobile payment methods for most purchases.` },
    { q: `Should I bring US dollars or euros as backup cash for ${c.name}?`, a: `Having some local currency in cash is generally more useful than foreign cash for everyday spending — exchange or withdraw a small amount on arrival to cover ${c.cashSituations}.` },
  ];
...
<head>
    ...
    <title>Card & Cashless Payment Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="Can you pay by card in ${c.name}? Free instant checker for card acceptance and where you still need cash, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/cashless-payment-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Card & Cashless Payment Checker for ${c.name}">
    <meta property="og:description" content="Instant free checker: ${headline}">
    <meta property="og:url" content="https://travelsmarterapp.com/cashless-payment-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/cashless-payment-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/cashless-payment-checker.png">
    <meta name="twitter:title" content="Card & Cashless Payment Checker for ${c.name}">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${c.cardAcceptance} card acceptance</span>
        <h1>Card & Cashless Payment Checker for ${c.name}</h1>
        <p>${headline}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Card & Cash Rules</button>
            </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="cashless-payment-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/cashless-payment-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'cashless_payment_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.cardAcceptance === 'low' ? ' warn' : data.result.cardAcceptance === 'moderate' ? ' caution' : '');
                ...
                btn.disabled = false;
                btn.textContent = 'Check Card & Cash Rules';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/cashless-payment-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'cashless_payment_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'cashless-payment-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-cashlesspayment-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `cashless-payment-checker-${c.slug}.html`);

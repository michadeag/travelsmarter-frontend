// Generates queue-culture-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-queueculture-page.js scripts/data-queueculture-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = QUEUE_LABELS[c.queueLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const queueLabel = QUEUE_LABELS[c.queueLevel];

  const faqs = [
    { q: `What's queueing like in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${queueLabel}" mean?`, a: c.queueLevel === 'strict' ? `In ${c.name}, orderly single-file lines are a strong norm, and cutting is genuinely noticed — take it seriously.` : c.queueLevel === 'orderly' ? `In ${c.name}, queuing is the norm and generally respected in everyday settings.` : c.queueLevel === 'flexible' ? `In ${c.name}, lines exist but are loosely observed — some jostling or filling gaps is normal and not considered impolite.` : `In ${c.name}, formal single-file lines aren't always the default at busy counters or stops — holding your position and being direct about your turn is the practical norm.` },
    { q: `What about banks or government offices?`, a: 'Formal contexts tend to be more orderly everywhere, regardless of the destination\'s general reputation — and where a ticket-number system exists, always take a number rather than relying on a visible physical line.' },
...
<head>
    ...
    <title>${c.name} Queue Culture Checker | TravelSmarter</title>
    <meta name="description" content="What's queueing like in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/queue-culture-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${queueLabel}</span>
        <h1>${c.name} Queue Culture Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Queue Culture</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with queueing tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${queueLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="queue-culture-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/queue-culture-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'queue_culture_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Queue Culture';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/queue-culture-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'queue_culture_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'queue-culture-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-queueculture-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `queue-culture-checker-${c.slug}.html`);

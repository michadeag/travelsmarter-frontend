// Generates punctuality-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-punctuality-page.js scripts/data-punctuality-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = PUNCTUALITY_LABELS[c.punctualityLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const punctualityLabel = PUNCTUALITY_LABELS[c.punctualityLevel];

  const faqs = [
    { q: `Is it OK to be late in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${punctualityLabel}" mean?`, a: c.punctualityLevel === 'strict' ? `In ${c.name}, on-time means on-time — arriving even a few minutes late without notice is genuinely noticed and can be seen as disrespectful.` : c.punctualityLevel === 'moderate' ? `In ${c.name}, a short grace period for casual social plans is normal, but real lateness is still noticed, especially in business settings.` : c.punctualityLevel === 'relaxed' ? `In ${c.name}, being 15-30 minutes late to informal social plans is common and not considered rude — though business meetings still expect closer-to-on-time arrival.` : `In ${c.name}, start times for social and even some casual plans are treated as approximate rather than fixed — visitors are expected to adjust their expectations.` },
    { q: `Does this apply to business meetings too?`, a: 'No — professional meetings run more punctually than social gatherings almost everywhere in the world, regardless of a country\'s general reputation. Default to arriving on time or a few minutes early for anything business-related.' },
...
<head>
    ...
    <title>${c.name} Punctuality & Time Culture Checker | TravelSmarter</title>
    <meta name="description" content="Is it OK to show up late in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/punctuality-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Punctuality & Time Culture Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/punctuality-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/punctuality-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/punctuality-checker.png">
    <meta name="twitter:title" content="${c.name} Punctuality & Time Culture Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${punctualityLabel}</span>
        <h1>${c.name} Punctuality & Time Culture Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Punctuality Norms</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with time-culture tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${punctualityLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="punctuality-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/punctuality-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'punctuality_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Punctuality Norms';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/punctuality-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'punctuality_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'punctuality-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-punctuality-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `punctuality-checker-${c.slug}.html`);

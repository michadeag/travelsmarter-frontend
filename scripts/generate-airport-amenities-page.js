// Generates airport-amenities-checker-{slug}.html pages, one per airport,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-airport-amenities-page.js scripts/data-airport-amenities.js
const fs = require('fs');
...
function headline(a) {
  return `${a.name}: ${a.freeWifi ? 'free WiFi' : 'no reliably free WiFi'}, ${LOUNGE_LABELS[a.loungeQuality]} lounges.`;
}
...
function render(a, allAirports) {
  const h = headline(a);
  const faqs = [
    { q: `Does ${a.name} have free WiFi?`, a: `${a.freeWifi ? 'Yes.' : 'Not reliably.'} ${a.note}` },
    { q: `How good are the lounges at ${a.name}?`, a: `Lounge quality here is ${LOUNGE_LABELS[a.loungeQuality]}. ${a.loungeQuality === 'excellent' || a.loungeQuality === 'good' ? 'A day pass, often purchasable without status or a premium ticket, can be well worth it for a longer layover.' : 'It\'s usually not worth counting on a lounge day pass here — a quiet gate area or sit-down restaurant is a more reliable plan.'}` },
    { q: `What's the one amenity worth seeking out at ${a.name}?`, a: a.topAmenity },
    { q: `Is ${a.name} a good airport for a long layover?`, a: a.loungeQuality === 'excellent' ? `Yes — ${a.name} is one of the better airports in the world to have a long layover in, with plenty to see and do.` : `It depends on how long you have — check the amenity above, and otherwise treat it as a functional connection rather than a destination in itself.` },
  ];
...
<head>
    ...
    <title>${a.name} WiFi & Amenities Guide | TravelSmarter</title>
    <meta name="description" content="Does ${a.name} have free WiFi and good lounges? Free instant checker with the top amenity to look for, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/airport-amenities-checker-${a.slug}.html">
    ...
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/airport-amenities-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/airport-amenities-checker.png">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${a.name} · ${LOUNGE_LABELS[a.loungeQuality]} lounges</span>
        <h1>${a.name} WiFi & Amenities Guide</h1>
        <p>${h}</p>
    </div>
...
${airportOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Airport Amenities</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with tips for a smoother layover at ${a.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="airport-amenities-checker.html">Check a different airport →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/airport-amenities-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'airport_amenities_checker_${a.slug}' });

                document.getElementById('result-headline').textContent = data.result.headline;
                document.getElementById('result-meta').textContent = data.result.topAmenity;
                ...
                btn.disabled = false;
                btn.textContent = 'Check Airport Amenities';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/airport-amenities-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'airport_amenities_checker_${a.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a2 = document.createElement('a');
                a2.href = url;
                a2.download = 'airport-amenities-checker-${a.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-airport-amenities-page.js <data-file.js>');
  process.exit(1);
}
...
airports.forEach(a => {
  ...
  const outPath = path.join(outDir, `airport-amenities-checker-${a.slug}.html`);

// Generates dress-code-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-dresscode-page.js scripts/data-dresscode-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  const label = DRESS_LABELS[c.dressCode];
  const faqs = [
    { q: `What's the dress code in ${c.name}?`, a: `${c.name}'s dress code is ${label}. ${c.dressNote || ''}`.trim() },
    { q: `Are there specific etiquette tips for ${c.name}?`, a: c.etiquetteNote || `No major etiquette pitfalls beyond general politeness — common sense and respect for local customs go a long way in ${c.name}.` },
    { q: `Do I need to cover up at religious sites in ${c.name}?`, a: c.dressNote ? c.dressNote : `${c.name} doesn't have strongly enforced dress requirements at most sites, but packing a light cover-up is always a safe habit when visiting any religious site worldwide.` },
    { q: `What should I pack to be safe in ${c.name}?`, a: `A lightweight scarf or sarong is the most versatile option — it packs small and can cover shoulders or legs on the spot for any temple, mosque, or church visit, regardless of what you're already wearing.` },
  ];
...
<head>
    ...
    <title>Dress Code & Etiquette Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="What's the dress code and etiquette in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/dress-code-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Dress Code & Etiquette Checker for ${c.name}">
    <meta property="og:description" content="Instant free checker: ${c.name}'s dress code is ${label}.">
    <meta property="og:url" content="https://travelsmarterapp.com/dress-code-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/dress-code-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/dress-code-checker.png">
    <meta name="twitter:title" content="Dress Code & Etiquette Checker for ${c.name}">
    <meta name="twitter:description" content="Instant free checker: ${c.name}'s dress code is ${label}.">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${c.dressCode}</span>
        <h1>Dress Code & Etiquette Checker for ${c.name}</h1>
        <p>${c.name}'s dress code is ${label}.</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Dress Code</button>
            </div>

            <div id="result">
                <div class="result-headline" id="result-headline"></div>
                <div class="result-meta" id="result-dress"></div>
                <div class="result-meta" id="result-etiquette"></div>

                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with packing and etiquette tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="dress-code-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/dress-code-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'dress_code_checker_${c.slug}' });

                document.getElementById('result-headline').textContent = data.result.headline;
                document.getElementById('result-dress').textContent = data.result.dressNote || '';
                document.getElementById('result-etiquette').textContent = data.result.etiquetteNote || '';
                ...
                btn.disabled = false;
                btn.textContent = 'Check Dress Code';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/dress-code-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'dress_code_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'dress-code-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-dresscode-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `dress-code-checker-${c.slug}.html`);

// Generates photography-permit-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-photographypermit-page.js scripts/data-photographypermit-countries.js
const fs = require('fs');
...
function headline(c) {
  return `${c.name}: ${STATUS_LABELS[c.status]} for photography and filming.`;
}
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Can I take photos and use a tripod in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${statusLabel}" mean?`, a: c.status === 'easy' ? `Casual photography and tripods are generally unrestricted in public — the usual exceptions apply for military sites and private property.` : c.status === 'moderate' ? `Some real restrictions exist — tripod permits at major landmarks, sensitivities around photographing people or religious sites, or specific sensitive zones — check the details above for this destination.` : `Meaningful legal risk exists around photographing government buildings, military sites, infrastructure, or people without consent — read the note above carefully before you shoot.` },
    { q: `Do I need a permit for commercial or professional filming in ${c.name}?`, a: `Almost universally, yes — a crew, lighting rig, or paid shoot typically needs a local permit regardless of how relaxed casual tourist photography is.` },
    { q: `Do these rules apply to every site in ${c.name}?`, a: `Not necessarily — rules can vary by specific museum, temple, or landmark, not just by country. Always check the exact site's current photography policy before you go.` },
  ];
...
<head>
    ...
    <title>${c.name} Photography & Filming Rules | TravelSmarter</title>
    <meta name="description" content="Can you take photos and use a tripod in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/photography-permit-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Photography & Filming Rules">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/photography-permit-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/photography-permit-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/photography-permit-checker.png">
    <meta name="twitter:title" content="${c.name} Photography & Filming Rules">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Photography & Filming Rules</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Photography Rules</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with photography tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="photography-permit-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/photography-permit-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'photography_permit_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Photography Rules';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/photography-permit-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'photography_permit_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'photography-permit-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-photographypermit-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `photography-permit-checker-${c.slug}.html`);

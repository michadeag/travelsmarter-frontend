// Generates emergency-number-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-emergency-page.js scripts/data-emergency-countries.js
const fs = require('fs');
const path = require('path');

function headline(c) {
  return c.unifiedNumber
    ? `In ${c.name}, dial ${c.unifiedNumber} for any emergency.`
    : `${c.name} uses separate numbers: ${c.police} for police, ${c.fire} for fire, ${c.ambulance} for ambulance.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const faqs = [
    { q: `What is the emergency number in ${c.name}?`, a: c.unifiedNumber ? `Dial ${c.unifiedNumber} for any emergency in ${c.name} — it reaches police, fire, and medical services.` : `${c.name} uses separate numbers by service: ${c.police} for police, ${c.fire} for fire, ${c.ambulance} for ambulance.` },
    { q: `Does 911 work in ${c.name}?`, a: c.unifiedNumber === '911' ? `Yes — ${c.name} uses 911, the same as the US.` : `No — 911 does not reach emergency services in ${c.name}. ${c.note}` },
    { q: `What should I do in a serious emergency as a US citizen in ${c.name}?`, a: `For serious situations (arrest, death, or crisis), contact the nearest US Embassy or Consulate — find current contact details at state.gov before you travel. The State Department's STEP program also lets you register your trip so the embassy can reach you in a crisis.` },
    { q: `Should I save these numbers before I travel to ${c.name}?`, a: `Yes — save the local emergency numbers to your phone before you land, since data or wifi may not be available exactly when you need them.` },
  ];
...
<head>
    ...
    <title>Emergency Number in ${c.name} | TravelSmarter</title>
    <meta name="description" content="What's the emergency number in ${c.name}? Free instant checker with police, fire, and ambulance numbers, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/emergency-number-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Emergency Number in ${c.name}">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/emergency-number-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/emergency-number-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/emergency-number-checker.png">
    <meta name="twitter:title" content="Emergency Number in ${c.name}">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name}${c.unifiedNumber ? ` · ${c.unifiedNumber}` : ''}</span>
        <h1>Emergency Number in ${c.name}</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Emergency Numbers</button>
            </div>
        ...
        <div class="card content-block">
            <h2>${c.name} emergency numbers at a glance</h2>
            <table class="limits-table">
                <tr><td>Police</td><td>${c.police}</td></tr>
                <tr><td>Fire</td><td>${c.fire}</td></tr>
                <tr><td>Ambulance</td><td>${c.ambulance}</td></tr>
            </table>
            <p>${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="emergency-number-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/emergency-number-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'emergency_number_checker_${c.slug}' });

                document.getElementById('result-headline').textContent = data.result.headline;
                document.getElementById('result-meta').textContent =
                    \`Police: \${data.result.police} · Fire: \${data.result.fire} · Ambulance: \${data.result.ambulance}\`;
                ...
                btn.disabled = false;
                btn.textContent = 'Check Emergency Numbers';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                ...
                gtag('event', 'generate_lead', { tool: 'emergency_number_checker_${c.slug}' });
...
if (!dataFile) {
  console.error('Usage: node generate-emergency-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `emergency-number-checker-${c.slug}.html`);

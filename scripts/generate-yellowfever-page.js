// Generates yellow-fever-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-yellowfever-page.js scripts/data-yellowfever-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = REQUIREMENT_LABELS[c.requirement];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const requirementLabel = REQUIREMENT_LABELS[c.requirement];

  const faqs = [
    { q: `Do I need a yellow fever certificate to enter ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${requirementLabel}" mean?`, a: c.requirement === 'not_required' ? `${c.name} has no legal entry requirement for yellow fever vaccination, regardless of where you're arriving from.` : c.requirement === 'conditional' ? `${c.name} only requires proof of vaccination if you're arriving from (or have recently transited) a country with risk of yellow fever transmission — irrelevant for most travelers flying in directly from a non-risk country.` : `${c.name} requires proof of vaccination for essentially every arriving traveler, regardless of origin — one of the stricter entry requirements globally.` },
    { q: `What happens if I show up without the certificate where it's required?`, a: `You can be denied boarding by the airline before you even leave, or quarantined, fined, or vaccinated on the spot on arrival — this is an actively enforced border requirement, not just general health advice.` },
    { q: `How is this different from general travel health advice for ${c.name}?`, a: `General health advisories cover what vaccines are recommended for your own protection. This is specifically about the legal document — the International Certificate of Vaccination or Prophylaxis — that a border official can require you to show before letting you in.` },
  ];
...
<head>
    ...
    <title>${c.name} Yellow Fever Certificate Requirement | TravelSmarter</title>
    <meta name="description" content="Do you need a yellow fever vaccination certificate to enter ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/yellow-fever-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Yellow Fever Certificate Requirement">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/yellow-fever-checker-${c.slug}.html">
    <meta property="og:image" content="https://travelsmarterapp.com/og-images/yellow-fever-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://travelsmarterapp.com/og-images/yellow-fever-checker.png">
    <meta name="twitter:title" content="${c.name} Yellow Fever Certificate Requirement">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${requirementLabel}</span>
        <h1>${c.name} Yellow Fever Certificate Requirement</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Requirement</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with yellow fever prep tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${requirementLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="yellow-fever-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/yellow-fever-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'yellow_fever_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Requirement';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/yellow-fever-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'yellow_fever_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'yellow-fever-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-yellowfever-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `yellow-fever-checker-${c.slug}.html`);

// Generates layover-checker-{slug}.html pages, one per airport,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-layover-page.js scripts/data-layover-airports.js
const fs = require('fs');
...
function render(a, allAirports) {
  const faqs = [
    { q: `What is the minimum connection time at ${a.name}?`, a: `As a general guideline: about ${a.mctDomestic} minutes for a domestic-to-domestic connection, ${a.mctMixed} minutes when one leg is international, and ${a.mctIntl} minutes for international-to-international connections. Always confirm your specific minimum with your airline, since it can vary by terminal and itinerary.` },
    { q: `Is ${a.name} a difficult airport for tight connections?`, a: a.notes },
    { q: `How much extra time should I add at ${a.name} for a comfortable connection?`, a: `Add about 30 minutes on top of the minimum connection time for a comfortable buffer — so roughly ${a.mctDomestic + 30} minutes for domestic-to-domestic, ${a.mctMixed + 30} minutes for a mixed connection, or ${a.mctIntl + 30} minutes for international-to-international.` },
    { q: `What should I do if my connection at ${a.name} is too tight?`, a: `Check in online and get your boarding pass before landing if possible, go straight to your gate if you only have carry-on luggage, and if you do miss the connection, head to your airline's transfer desk or app rather than a general check-in line for faster rebooking.` },
  ];
...
<head>
    ...
    <title>${a.name} Layover Checker — Is My Connection Long Enough? | TravelSmarter</title>
    <meta name="description" content="Is your layover at ${a.name} long enough? Free instant checker with real minimum connection times, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/layover-checker-${a.slug}.html">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${a.name} Layover Checker</h1>
        <p>Enter your connection type and layover time to see if you're cutting it close at ${a.name} — free, no sign-up required.</p>
    </div>

    <div class="container">
        <div class="card">
            <div id="form-section">
                <label for="airport">Connecting airport</label>
                <select id="airport">
${airportOptions}
                </select>
                ...
                <div class="grid-2">
                    <div>
                        ...
                        <select id="connectionType">
                            ...
                            <option value="mixed">International ↔ Domestic</option>
                            <option value="international">International → International</option>
                        </select>
                    </div>
                    <div>
                        <label for="availableMinutes">Layover time (minutes)</label>
                        <input type="number" id="availableMinutes" value="90" min="0" step="5">
                    </div>
                </div>

                <div id="calc-alert" class="alert alert-error"></div>
                <button class="btn" id="calc-btn" onclick="calculate()">Check My Layover</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full report?</strong> Get a PDF with what to do if your connection is tight — free.</p>
        ...
        <div class="card content-block">
            ...
            <table class="limits-table">
                <tr><td>Domestic → Domestic</td><td>~${a.mctDomestic} min minimum</td></tr>
                <tr><td>International ↔ Domestic</td><td>~${a.mctMixed} min minimum</td></tr>
                <tr><td>International → International</td><td>~${a.mctIntl} min minimum</td></tr>
            </table>
            <p>${a.notes}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="layover-checker.html">Check a different airport →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            const availableMinutes = document.getElementById('availableMinutes').value;
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/layover-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ airport, connectionType, availableMinutes }),
                });
                const data = await res.json();
                if (!data.success) throw new Error(data.error || 'Something went wrong');

                lastResult = data.result;
                gtag('event', 'tool_calculate', { tool: 'layover_checker_${a.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check My Layover';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/layover-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                        ...
                        connectionType: lastResult.connectionType,
                        availableMinutes: lastResult.availableMinutes,
                    }),
                });
                if (!res.ok) {
                ...
                gtag('event', 'generate_lead', { tool: 'layover_checker_${a.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a2 = document.createElement('a');
                a2.href = url;
                a2.download = 'layover-checker-${a.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-layover-page.js <data-file.js>');
  process.exit(1);
}
...
airports.forEach(a => {
  ...
  const outPath = path.join(outDir, `layover-checker-${a.slug}.html`);

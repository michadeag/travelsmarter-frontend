// Generates water-safety-checker-{slug}.html pages, one per country,
...
// Run: node scripts/generate-water-page.js scripts/data-water-countries.js
const fs = require('fs');
...
function headline(c) {
  if (c.status === 'safe') return `${c.name}: tap water is safe to drink.`;
  if (c.status === 'caution') return `${c.name}: tap water is treated but bottled or filtered water is the safer default.`;
  return `${c.name}: stick to bottled or filtered water — tap water isn't recommended.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const faqs = [
    { q: `Is tap water safe to drink in ${c.name}?`, a: c.note },
    { q: `Is it safe to have ice in drinks in ${c.name}?`, a: c.status === 'safe' ? `Yes — since tap water is safe in ${c.name}, ice made from it is generally fine too.` : `Extend the same caution to ice as to tap water in ${c.name} — it's easy to overlook, but ice carries the same risk unless you know it's made from purified water.` },
    { q: `Can I brush my teeth with tap water in ${c.name}?`, a: `Brushing your teeth with tap water is usually fine even where drinking it isn't recommended, but if you want to be extra cautious in ${c.name}, use bottled water for that too.` },
    { q: `Where can I buy safe drinking water in ${c.name}?`, a: `Sealed bottled water from a reputable brand is widely available and the simplest safe option in ${c.name} — just check the seal is intact before drinking.` },
  ];
...
<head>
    ...
    <title>Is Tap Water Safe to Drink in ${c.name}? | TravelSmarter</title>
    <meta name="description" content="Is tap water safe in ${c.name}? Free instant checker with practical tips, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/water-safety-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Is Tap Water Safe to Drink in ${c.name}?">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/water-safety-checker-${c.slug}.html">
    <meta property="og:image" content="https://travelsmarterapp.com/og-images/water-safety-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://travelsmarterapp.com/og-images/water-safety-checker.png">
    <meta name="twitter:title" content="Is Tap Water Safe to Drink in ${c.name}?">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>Is Tap Water Safe to Drink in ${c.name}?</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Water Safety</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with practical tips on ice, brushing teeth, and bottled water for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="water-safety-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/water-safety-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'water_safety_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Water Safety';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/water-safety-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'water_safety_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'water-safety-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-water-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `water-safety-checker-${c.slug}.html`);

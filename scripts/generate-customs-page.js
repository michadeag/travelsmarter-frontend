// Generates customs-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-customs-page.js scripts/data-customs-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const faqs = isPuertoRico ? [
    { q: `Do I need to go through customs returning from ${c.name}?`, a: `No — ${c.name} is US territory, so returning to the mainland is treated like a domestic flight with no customs process.` },
    ...
    { q: `What should I watch out for bringing things back from ${c.name}?`, a: c.watchItem },
    { q: `Do I need to declare gifts and items still in checked luggage from ${c.name}?`, a: `Yes — everything you're bringing back needs to be declared, including gifts and anything packed in checked bags, not just what you're carrying on you.` },
  ];
...
<head>
    ...
    <title>Duty-Free Allowance from ${c.name} | Customs Guide | TravelSmarter</title>
    <meta name="description" content="How much can you bring back duty-free from ${c.name}? Free instant checker with your US customs allowance, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/customs-checker-${c.slug}.html">
    ...
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/customs-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/customs-checker.png">
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Customs Allowance</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with tips for clearing customs from ${c.name} smoothly — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="customs-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/customs-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'customs_checker_${c.slug}' });

                document.getElementById('result-headline').textContent = data.result.headline;
                document.getElementById('result-meta').textContent = data.result.watchItem;
                ...
                btn.disabled = false;
                btn.textContent = 'Check Customs Allowance';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/customs-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'customs_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'customs-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-customs-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `customs-checker-${c.slug}.html`);

// Generates solo-dining-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-solodining-page.js scripts/data-solodining-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = SOLO_LABELS[c.soloLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const soloLabel = SOLO_LABELS[c.soloLevel];

  const faqs = [
    { q: `Is it normal to eat alone at a restaurant in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${soloLabel}" mean?`, a: c.soloLevel === 'very-common' ? `In ${c.name}, solo dining is genuinely well-accommodated — counter seating and solo-friendly formats are common by design, not just tolerated.` : c.soloLevel === 'common' ? `In ${c.name}, solo dining is normal and unremarkable — nobody will think twice about a table of one.` : c.soloLevel === 'workable' ? `In ${c.name}, solo dining is entirely workable, but the overall dining culture skews social or group-oriented, so you may notice you're less common at certain formats, especially dinner.` : `In ${c.name}, dining out is strongly a social or couples-oriented activity — solo diners are welcome but may find genuine friction, especially at dinner.` },
    { q: `Where's the easiest place to eat alone?`, a: 'Counter seating, food halls, and street food stalls are near-universally solo-friendly, even in destinations where sit-down dining skews social — lean on these for an easy, unremarkable solo meal.' },
...
<head>
    ...
    <title>${c.name} Solo Dining Checker | TravelSmarter</title>
    <meta name="description" content="Is it normal to eat alone at a restaurant in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/solo-dining-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Solo Dining Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/solo-dining-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/solo-dining-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/solo-dining-checker.png">
    <meta name="twitter:title" content="${c.name} Solo Dining Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${soloLabel}</span>
        <h1>${c.name} Solo Dining Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Solo Dining Norms</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with solo dining tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${soloLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="solo-dining-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/solo-dining-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'solo_dining_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Solo Dining Norms';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/solo-dining-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'solo_dining_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'solo-dining-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-solodining-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `solo-dining-checker-${c.slug}.html`);

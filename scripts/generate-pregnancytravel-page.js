// Generates pregnancy-travel-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-pregnancytravel-page.js scripts/data-pregnancytravel-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Is it safe to travel to ${c.name} while pregnant?`, a: `${h} ${c.note}` },
    { q: `What does "${riskLabel}" mean?`, a: c.riskLevel === 'minimal' ? `${c.name} has no established local Zika transmission risk — standard prenatal travel precautions apply as they would for any trip.` : c.riskLevel === 'low' ? `${c.name} has limited or historical local Zika transmission — standard strict mosquito-bite precautions are recommended for pregnant travelers.` : `${c.name} has a documented history of local Zika transmission — the CDC has recommended pregnant travelers discuss trips here with their doctor and consider postponing non-essential travel.` },
    { q: `What should I do before booking, regardless of the risk level?`, a: DISCLAIMER },
    { q: `What else should I check before flying while pregnant?`, a: "Most airlines restrict flying after a certain point in pregnancy (commonly around 36 weeks for domestic, earlier for international) — check your specific airline's policy and consider a doctor's letter for later-term travel. Travel insurance that explicitly covers pregnancy-related complications is also worth the extra cost." },
  ];
...
<head>
    ...
    <title>${c.name} Pregnancy Travel Risk | TravelSmarter</title>
    <meta name="description" content="Is it safe to travel to ${c.name} while pregnant? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/pregnancy-travel-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Pregnancy Travel Risk</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Pregnancy Travel Risk</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with pregnancy travel tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="pregnancy-travel-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/pregnancy-travel-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'pregnancy_travel_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Pregnancy Travel Risk';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/pregnancy-travel-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'pregnancy_travel_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'pregnancy-travel-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-pregnancytravel-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `pregnancy-travel-checker-${c.slug}.html`);

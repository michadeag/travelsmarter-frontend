// Generates lgbtq-travel-safety-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-lgbtqsafety-page.js scripts/data-lgbtqsafety-countries.js
const fs = require('fs');
...
function headline(c) {
  return `${c.name}: ${STATUS_LABELS[c.status]}.`;
}
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Is ${c.name} safe for LGBTQ+ travelers?`, a: `${h} ${c.note}` },
    { q: `What does "${statusLabel}" mean?`, a: c.status === 'legal_and_protected' ? `Same-sex activity is legal and relationships are legally recognized (marriage or civil union), with anti-discrimination protections in place.` : c.status === 'legal' ? `Same-sex activity is legal, but there's little or no formal legal recognition of relationships — social acceptance varies, as noted above.` : `Same-sex sexual activity is illegal under local law — this carries real legal risk for travelers, not just residents.` },
    { q: `Is this a live, real-time rating?`, a: DISCLAIMER },
    { q: `Where can I check current, city-level detail?`, a: `Equaldex.com and ILGA World's legal mapping report are specialized, regularly updated sources — check them before you book and again shortly before you fly, especially for destinations marked "Legal" or "Criminalized" above.` },
  ];
...
<head>
    ...
    <title>Is ${c.name} Safe for LGBTQ+ Travelers? | TravelSmarter</title>
    <meta name="description" content="Is ${c.name} safe for LGBTQ+ travelers? Free instant legal status checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/lgbtq-travel-safety-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Is ${c.name} Safe for LGBTQ+ Travelers?">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/lgbtq-travel-safety-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/lgbtq-travel-safety-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/lgbtq-travel-safety-checker.png">
    <meta name="twitter:title" content="Is ${c.name} Safe for LGBTQ+ Travelers?">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>Is ${c.name} Safe for LGBTQ+ Travelers?</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check LGBTQ+ Travel Safety</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with pre-trip safety tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="lgbtq-travel-safety-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/lgbtq-travel-safety-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'lgbtq_travel_safety_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check LGBTQ+ Travel Safety';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/lgbtq-travel-safety-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'lgbtq_travel_safety_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'lgbtq-travel-safety-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-lgbtqsafety-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `lgbtq-travel-safety-checker-${c.slug}.html`);

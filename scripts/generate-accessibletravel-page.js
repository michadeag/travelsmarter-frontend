// Generates accessible-travel-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-accessibletravel-page.js scripts/data-accessibletravel-countries.js
const fs = require('fs');
...
function headline(c) {
  return `${c.name}: ${LEVEL_LABELS[c.level]}.`;
}
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Is ${c.name} wheelchair accessible?`, a: `${h} ${c.note}` },
    { q: `What does "${levelLabel}" mean?`, a: c.level === 'excellent' ? `Strong accessibility legislation combined with consistent modern infrastructure investment — one of the more reliable destinations for wheelchair users.` : c.level === 'good' ? `Modern hotels, transit, and major attractions are generally accessible, with real gaps mainly in historic centers or older buildings.` : c.level === 'moderate' ? `Accessible pockets exist (resorts, newer developments, major hotels) alongside significant general infrastructure gaps — research your specific itinerary carefully.` : `Accessible infrastructure is the exception rather than the norm here — international resorts and hotels are a more reliable bet than general city infrastructure.` },
    { q: `Is this a guarantee for a specific hotel or attraction?`, a: DISCLAIMER },
    { q: `What should I do before booking?`, a: `Call or email specific hotels and attractions directly to confirm accessibility for your exact needs, book airline/airport wheelchair assistance in advance rather than at the airport, and consider an accessible-travel specialist tour operator for your destination.` },
  ];
...
<head>
    ...
    <title>Is ${c.name} Wheelchair Accessible? | TravelSmarter</title>
    <meta name="description" content="Is ${c.name} wheelchair accessible? Free instant accessibility checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/accessible-travel-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Is ${c.name} Wheelchair Accessible?">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/accessible-travel-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/accessible-travel-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/accessible-travel-checker.png">
    <meta name="twitter:title" content="Is ${c.name} Wheelchair Accessible?">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${levelLabel}</span>
        <h1>Is ${c.name} Wheelchair Accessible?</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Accessibility</button>
            </div>

            <div id="result">
                <div class="result-headline" id="result-headline"></div>
                <div class="result-meta" id="result-meta"></div>
                <div class="disclaimer-box" id="result-disclaimer"></div>

                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with accessible-travel prep tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${levelLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="accessible-travel-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/accessible-travel-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'accessible_travel_checker_${c.slug}' });
                ...
                document.getElementById('result-disclaimer').textContent = data.result.disclaimer;
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/accessible-travel-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'accessible_travel_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'accessible-travel-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-accessibletravel-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `accessible-travel-checker-${c.slug}.html`);

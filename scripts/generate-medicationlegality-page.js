// Generates medication-legality-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-medicationlegality-page.js scripts/data-medicationlegality-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  const restrictionLevelLabel = RESTRICTION_LABELS[c.restrictionLevel];
  const headline = `${c.name}'s medication import restrictions are ${restrictionLevelLabel}. Commonly restricted: ${c.commonlyRestricted}.`;

  const faqs = [
    { q: `Can I bring my prescription medication to ${c.name}?`, a: `${c.name}'s medication import restrictions are ${restrictionLevelLabel} ${c.note}` },
    { q: `What medications are commonly restricted in ${c.name}?`, a: `Commonly restricted: ${c.commonlyRestricted}.` },
    { q: `What documents should I carry for medication in ${c.name}?`, a: "Keep medication in its original, pharmacy-labeled packaging, and carry both your prescription and a doctor's letter (on letterhead, listing the medication, dosage, and medical reason)." },
    { q: `Is this medical or legal advice?`, a: `No — this is general travel guidance, not medical or legal advice. Always confirm current rules with ${c.name}'s embassy or consulate for your specific medication before you travel.` },
  ];
...
<head>
    ...
    <title>Medication Legality Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="Can you bring your prescription medication to ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/medication-legality-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Medication Legality Checker for ${c.name}">
    <meta property="og:description" content="Instant free checker: ${headline}">
    <meta property="og:url" content="https://travelsmarterapp.com/medication-legality-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/medication-legality-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/medication-legality-checker.png">
    <meta name="twitter:title" content="Medication Legality Checker for ${c.name}">
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .disclaimer { color:#9ca3af; font-size:12.5px; margin-top:10px; }
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${c.restrictionLevel} restrictions</span>
        <h1>Medication Legality Checker for ${c.name}</h1>
        <p>${headline}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Medication Rules</button>
            </div>
            ...
            <p class="disclaimer">General travel guidance, not medical or legal advice — always confirm with the destination's embassy for your specific medication.</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="medication-legality-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/medication-legality-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'medication_legality_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.restrictionLevel === 'high' ? ' warn' : data.result.restrictionLevel === 'moderate' ? ' caution' : '');
                ...
                btn.disabled = false;
                btn.textContent = 'Check Medication Rules';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/medication-legality-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'medication_legality_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'medication-legality-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-medicationlegality-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `medication-legality-checker-${c.slug}.html`);

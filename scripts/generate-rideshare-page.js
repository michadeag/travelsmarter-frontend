// Generates rideshare-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-rideshare-page.js scripts/data-rideshare-countries.js
const fs = require('fs');
...
function headline(c) {
  return `In ${c.name}: ${STATUS_LABELS[c.status]}${c.status !== 'full' ? ` — use ${c.localAlternative} instead` : ''}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const faqs = [
    { q: `Does Uber work in ${c.name}?`, a: c.status === 'full' ? `Yes — Uber operates normally throughout ${c.name}.` : c.status === 'limited' ? `Uber has limited availability in ${c.name}. ${c.note}` : `No — Uber does not operate in ${c.name}. ${c.note}` },
    { q: `What ride-hailing app should I use in ${c.name}?`, a: `${c.localAlternative} is the recommended app to use in ${c.name}.` },
    { q: `Why doesn't Uber work the same everywhere?`, a: `Uber has exited or been restricted in several markets due to local regulations or competitive sales — for example, it sold its Southeast Asia business to Grab and its China business to Didi, and has faced regulatory bans in a handful of European countries.` },
    { q: `Should I download the local app before I arrive in ${c.name}?`, a: `Yes — download ${c.localAlternative} and set up payment before you land, since some apps require a local phone number or work poorly without a data connection right at the airport.` },
  ];
...
<head>
    ...
    <title>Does Uber Work in ${c.name}? | TravelSmarter</title>
    <meta name="description" content="Does Uber work in ${c.name}? Free instant checker with the local ride-hailing app to use, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/rideshare-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Does Uber Work in ${c.name}?">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/rideshare-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/rideshare-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/rideshare-checker.png">
    <meta name="twitter:title" content="Does Uber Work in ${c.name}?">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>Does Uber Work in ${c.name}?</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Rideshare Availability</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with tips for getting around ${c.name} smoothly — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="rideshare-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/rideshare-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'rideshare_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Rideshare Availability';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/rideshare-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'rideshare_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'rideshare-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-rideshare-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `rideshare-checker-${c.slug}.html`);

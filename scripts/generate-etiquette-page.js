// Generates etiquette-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-etiquette-page.js scripts/data-etiquette-countries.js
const fs = require('fs');
const path = require('path');

const TABOO_LABELS = {
  high: "high — etiquette missteps are commonly noticed and can genuinely offend, so it's worth learning the basics before you go",
  moderate: 'moderate — locals are generally forgiving of foreigners, but a few specific customs are worth knowing',
  low: 'low — norms are relaxed and foreigners get plenty of leeway',
};
...
function render(c, allCountries) {
  const tabooLevelLabel = TABOO_LABELS[c.tabooLevel];
  const headline = `${c.name}'s etiquette sensitivity is ${tabooLevelLabel}. Most important: ${c.keyRule}`;

  const faqs = [
    { q: `What's the biggest etiquette mistake to avoid in ${c.name}?`, a: c.keyRule },
    { q: `How careful do I need to be about etiquette in ${c.name}?`, a: `${c.name}'s etiquette sensitivity is ${tabooLevelLabel}` },
    { q: `Are there other customs worth knowing about ${c.name}?`, a: c.note },
    { q: `Will locals be upset if I make a mistake in ${c.name}?`, a: c.tabooLevel === 'high' ? `A genuine misstep is more likely to be noticed here than in more relaxed destinations, but a good-faith effort and a quick apology go a long way.` : `Locals are generally understanding of foreign visitors — a little awareness helps, but nobody expects perfection.` },
  ];
...
<head>
    ...
    <title>Local Etiquette & Taboos Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="What are the key etiquette rules and taboos in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/etiquette-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Local Etiquette & Taboos Checker for ${c.name}">
    <meta property="og:description" content="Instant free checker: ${headline}">
    <meta property="og:url" content="https://travelsmarterapp.com/etiquette-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/etiquette-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/etiquette-checker.png">
    <meta name="twitter:title" content="Local Etiquette & Taboos Checker for ${c.name}">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${c.tabooLevel} sensitivity</span>
        <h1>Local Etiquette & Taboos Checker for ${c.name}</h1>
        <p>${headline}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Etiquette Rules</button>
            </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="etiquette-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/etiquette-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'etiquette_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.tabooLevel === 'high' ? ' warn' : data.result.tabooLevel === 'moderate' ? ' caution' : '');
                ...
                btn.disabled = false;
                btn.textContent = 'Check Etiquette Rules';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/etiquette-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'etiquette_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'etiquette-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-etiquette-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `etiquette-checker-${c.slug}.html`);

// Generates sim-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-sim-page.js scripts/data-sim-countries.js
const fs = require('fs');
...
function headline(c) {
  return `In ${c.name}: ${BEST_OPTION_LABELS[c.bestOption]}${c.registrationRequired ? ' — passport/ID registration is required to buy a local SIM' : ''}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const faqs = [
    { q: `Should I get an eSIM or a physical SIM for ${c.name}?`, a: c.bestOption === 'esim' ? `An eSIM is the better choice for ${c.name}. ${c.note}` : `A local physical SIM is the better choice for ${c.name}. ${c.note}` },
    { q: `Do I need to register my passport to buy a SIM in ${c.name}?`, a: c.registrationRequired ? `Yes — ${c.name} requires passport or ID registration to activate a local physical SIM.` : `No — ${c.name} doesn't require passport/ID registration to get connected.` },
    { q: `Where should I buy my SIM or eSIM for ${c.name}?`, a: c.bestOption === 'esim' ? `Buy your eSIM online before you fly (Airalo and similar marketplaces cover ${c.name}) and install the profile while you still have WiFi — it activates the moment you land.` : `Buy your physical SIM at the airport arrivals hall in ${c.name} — most major carriers have a kiosk right past immigration, and staff can handle the registration paperwork for you.` },
    { q: `Will my phone work in ${c.name} without changing anything?`, a: `Only if you're fine paying your home carrier's international roaming rates — check your plan first, since those can add up fast compared to a local SIM or eSIM.` },
  ];
...
<head>
    ...
    <title>SIM or eSIM for ${c.name}? Connectivity Guide | TravelSmarter</title>
    <meta name="description" content="Should you get a local SIM or an eSIM for ${c.name}, and do you need to register with your passport? Free instant checker plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/sim-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="SIM or eSIM for ${c.name}?">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/sim-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/sim-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/sim-checker.png">
    <meta name="twitter:title" content="SIM or eSIM for ${c.name}?">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${c.registrationRequired ? 'ID registration required' : 'No registration hassle'}</span>
        <h1>SIM or eSIM for ${c.name}?</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Connectivity Options</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with tips for staying connected in ${c.name} smoothly — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="sim-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/sim-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'sim_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Connectivity Options';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/sim-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'sim_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'sim-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-sim-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `sim-checker-${c.slug}.html`);

// Generates onward-travel-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-onwardtravel-page.js scripts/data-onwardtravel-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Does ${c.name} require proof of onward or return travel?`, a: `${h} ${c.note}` },
    { q: `What does "${enforcementLabel}" mean?`, a: c.enforcementLevel === 'rare' ? `${c.name} essentially never checks for this with typical tourists — it's not a real concern.` : c.enforcementLevel === 'inconsistent' ? `${c.name} sometimes asks for proof of onward travel, but it's not universal — some travelers get asked, most don't.` : `${c.name} routinely enforces this, often right at the airline check-in counter before you even board, not just at the border.` },
    { q: `What counts as proof of onward travel?`, a: "A return or onward flight, bus, or train ticket showing you'll leave before your visa/entry allowance expires. A fully refundable booking works too — many travelers book one and cancel it after boarding." },
    { q: `What if I'm traveling one-way with no fixed return date?`, a: "A fully refundable flight booking (cancel it after you board) is the simplest fix. Proof-of-onward-travel rental services generate a real, verifiable-looking reservation for a small fee if you have no fixed return date." },
...
<head>
    ...
    <title>${c.name} Proof of Onward Travel Checker | TravelSmarter</title>
    <meta name="description" content="Does ${c.name} require proof of onward or return travel? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/onward-travel-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Proof of Onward Travel Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/onward-travel-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/onward-travel-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/onward-travel-checker.png">
    <meta name="twitter:title" content="${c.name} Proof of Onward Travel Checker">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Proof of Onward Travel Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Onward Travel Requirement</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with onward travel tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="onward-travel-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/onward-travel-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'onward_travel_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Onward Travel Requirement';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/onward-travel-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'onward_travel_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'onward-travel-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-onwardtravel-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `onward-travel-checker-${c.slug}.html`);

// Generates vpn-censorship-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-vpncensorship-page.js scripts/data-vpncensorship-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const vpnLabel = VPN_LABELS[c.vpnLegal];
  const headline = `${c.name}'s internet censorship level is ${levelLabel}.`;

  const faqs = [
    { q: `Is the internet censored in ${c.name}?`, a: `${c.name}'s internet censorship level is ${levelLabel}. ${c.note}` },
    { q: `Do I need a VPN for ${c.name}?`, a: c.level === 'heavy' ? `Yes — a VPN is genuinely necessary in ${c.name} for accessing commonly-blocked apps and sites.` : c.level === 'moderate' ? `A VPN can help in ${c.name} for accessing specific restricted services, though most everyday apps work fine without one.` : `A VPN isn't essential in ${c.name} for typical tourist use, though it's still useful for general privacy on public wifi.` },
    { q: `Is using a VPN legal in ${c.name}?`, a: `In ${c.name}, ${vpnLabel}.` },
    { q: `Should I set up my VPN before I arrive in ${c.name}?`, a: c.level === 'heavy' ? `Yes — VPN provider websites and app stores are often themselves restricted once you're already in ${c.name}, so download and test your VPN before you fly.` : `It's good practice to set up and test your VPN before any trip, though it's less critical for ${c.name} specifically.` },
  ];
...
<head>
    ...
    <title>Internet Censorship & VPN Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="Is the internet censored in ${c.name}? Free instant checker for blocked apps, VPN necessity, and VPN legality, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/vpn-censorship-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${c.level} censorship</span>
        <h1>Internet Censorship & VPN Checker for ${c.name}</h1>
        <p>${headline}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Internet & VPN Status</button>
            </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="vpn-censorship-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/vpn-censorship-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'vpn_censorship_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.level === 'heavy' ? ' warn' : data.result.level === 'moderate' ? ' caution' : '');
                document.getElementById('result-meta').textContent = \`\${data.result.note} VPN legality: \${data.result.vpnLabel}.\`;
                ...
                btn.disabled = false;
                btn.textContent = 'Check Internet & VPN Status';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/vpn-censorship-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'vpn_censorship_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'vpn-censorship-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-vpncensorship-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `vpn-censorship-checker-${c.slug}.html`);

// Generates currency-convertibility-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-currencyconvertibility-page.js scripts/data-currencyconvertibility-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = CONVERTIBILITY_LABELS[c.convertibility];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const convertibilityLabel = CONVERTIBILITY_LABELS[c.convertibility];

  const faqs = [
    { q: `Can I exchange leftover ${c.name} currency after my trip?`, a: `${h} ${c.note}` },
    { q: `What does "${convertibilityLabel}" mean?`, a: c.convertibility === 'freely-convertible' ? `${c.name}'s currency is a major, widely-traded currency — exchange leftover cash anywhere, including at home, with no rush.` : c.convertibility === 'convertible-locally' ? `${c.name}'s currency is genuinely convertible, but realistically only within the country or region — convert or spend it before you leave.` : c.convertibility === 'restricted' ? `${c.name}'s currency has official export limits, capital controls, or proof-of-exchange requirements — confirm current rules before assuming you can convert freely.` : `${c.name}'s currency is essentially worthless outside the country in practice — spend it or it becomes a souvenir.` },
    { q: `Should I keep my exchange receipts?`, a: 'Yes, especially for "restricted" currencies — some require proof of the original exchange to convert cash back, so keep receipts if you\'re carrying a meaningful amount.' },
...
<head>
    ...
    <title>${c.name} Leftover Currency Checker | TravelSmarter</title>
    <meta name="description" content="Can you exchange leftover ${c.name} currency after your trip? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/currency-convertibility-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Leftover Currency Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/currency-convertibility-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/currency-convertibility-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/currency-convertibility-checker.png">
    <meta name="twitter:title" content="${c.name} Leftover Currency Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${convertibilityLabel}</span>
        <h1>${c.name} Leftover Currency Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Currency Convertibility</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with currency tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${convertibilityLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="currency-convertibility-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/currency-convertibility-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'currency_convertibility_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Currency Convertibility';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/currency-convertibility-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'currency_convertibility_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'currency-convertibility-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-currencyconvertibility-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `currency-convertibility-checker-${c.slug}.html`);

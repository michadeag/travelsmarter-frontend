// Generates travel-advisory-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-traveladvisory-page.js scripts/data-traveladvisory-countries.js
const fs = require('fs');
...
function headline(c) {
  return `${c.name}: Level ${c.level} — ${LEVEL_LABELS[c.level]}.`;
}
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Is ${c.name} safe to travel to?`, a: `${h} ${c.reason}` },
    { q: `What does "Level ${c.level} — ${levelLabel}" mean?`, a: c.level === 1 ? `Level 1 is the lowest advisory level — it means normal precautions are enough, the same baseline awareness you'd use anywhere.` : c.level === 2 ? `Level 2 means there are specific, known risk factors worth extra awareness — most travelers visit safely by following local guidance and common-sense precautions.` : c.level === 3 ? `Level 3 means real, standing risk factors exist and travel should be carefully reconsidered — check official guidance closely before committing.` : `Level 4 is the highest advisory level, meaning travel is advised against entirely.` },
    { q: `Is this a live, real-time rating?`, a: DISCLAIMER },
    { q: `What should I do before I travel to ${c.name}?`, a: `Check your own government's official current travel advisory (e.g. travel.state.gov for US citizens), enroll in a traveler registration program if one exists, and save your embassy or consulate's emergency contact details before you go.` },
  ];
...
<head>
    ...
    <title>Is ${c.name} Safe to Travel To? | Travel Advisory Checker | TravelSmarter</title>
    <meta name="description" content="Is ${c.name} safe to travel to right now? Free instant travel advisory checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/travel-advisory-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · Level ${c.level}</span>
        <h1>Is ${c.name} Safe to Travel To?</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Travel Advisory</button>
            </div>
        ...
        <div class="card content-block">
            ...
            <p><strong>Level ${c.level} of 4 — ${levelLabel}.</strong> ${c.reason}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="travel-advisory-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/travel-advisory-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'travel_advisory_checker_${c.slug}' });

                document.getElementById('result-headline').textContent = data.result.headline;
                document.getElementById('result-meta').textContent = data.result.reason;
                ...
                btn.disabled = false;
                btn.textContent = 'Check Travel Advisory';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/travel-advisory-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'travel_advisory_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'travel-advisory-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-traveladvisory-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `travel-advisory-checker-${c.slug}.html`);

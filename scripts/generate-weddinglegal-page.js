// Generates wedding-legal-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-weddinglegal-page.js scripts/data-weddinglegal-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = COMPLEXITY_LABELS[c.complexityLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const complexityLabel = COMPLEXITY_LABELS[c.complexityLevel];

  const faqs = [
    { q: `Can I legally get married in ${c.name} as a foreigner?`, a: `${h} ${c.note}` },
    { q: `What does "${complexityLabel}" mean?`, a: c.complexityLevel === 'straightforward' ? `${c.name} has minimal or no residency requirement and fast processing — a genuinely popular legal destination-wedding market.` : c.complexityLevel === 'moderate' ? `${c.name} has real paperwork and/or a notice period, but it's manageable with advance planning or a local wedding planner.` : `${c.name} has significant residency requirements, extensive documentation, or marriage law tied to religion — most couples here do a symbolic ceremony and marry legally at home instead.` },
    { q: `What if legal marriage here looks too complicated?`, a: `A popular, low-stress option is marrying legally at home first (or after) and having a purely symbolic ceremony at your destination — many couples do exactly this, especially in destinations marked "complex" here.` },
    { q: `Will the marriage be recognized back home?`, a: DISCLAIMER },
  ];
...
<head>
    ...
    <title>${c.name} Destination Wedding Legal Requirements | TravelSmarter</title>
    <meta name="description" content="Can you legally get married in ${c.name} as a foreigner? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/wedding-legal-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Destination Wedding Legal Requirements">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/wedding-legal-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/wedding-legal-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/wedding-legal-checker.png">
    <meta name="twitter:title" content="${c.name} Destination Wedding Legal Requirements">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${complexityLabel}</span>
        <h1>${c.name} Destination Wedding Legal Requirements</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Legal Requirements</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with destination wedding tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${complexityLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="wedding-legal-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/wedding-legal-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'wedding_legal_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Legal Requirements';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/wedding-legal-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'wedding_legal_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'wedding-legal-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-weddinglegal-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `wedding-legal-checker-${c.slug}.html`);

// Generates restaurant-reservation-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-restaurantreservation-page.js scripts/data-restaurantreservation-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = RESERVATION_LABELS[c.reservationLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const reservationLabel = RESERVATION_LABELS[c.reservationLevel];

  const faqs = [
    { q: `Do I need a reservation to eat out in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${reservationLabel}" mean?`, a: c.reservationLevel === 'essential' ? `In ${c.name}, reservations are strongly expected for good dining — booking ahead is the norm, not the exception, especially for anywhere well-reviewed.` : c.reservationLevel === 'recommended' ? `In ${c.name}, walking in often works, but reserving ahead — especially for dinner or on weekends — avoids a real wait at popular restaurants.` : c.reservationLevel === 'flexible' ? `In ${c.name}, there's a genuine mix — casual walk-in dining and reservation-only fine dining coexist, with no single dominant norm.` : `In ${c.name}, walk-in culture dominates for most dining — reservations are rarely needed outside a small number of higher-end restaurants.` },
    { q: `What if I have a specific restaurant in mind?`, a: 'Check that restaurant\'s own booking policy directly rather than relying on the destination\'s general culture — any genuinely popular or newly opened place can require a reservation regardless of local norms.' },
...
<head>
    ...
    <title>${c.name} Restaurant Reservation Checker | TravelSmarter</title>
    <meta name="description" content="Do you need a reservation to eat out in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/restaurant-reservation-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Restaurant Reservation Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/restaurant-reservation-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/restaurant-reservation-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/restaurant-reservation-checker.png">
    <meta name="twitter:title" content="${c.name} Restaurant Reservation Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${reservationLabel}</span>
        <h1>${c.name} Restaurant Reservation Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Reservation Norms</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with dining tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${reservationLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="restaurant-reservation-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/restaurant-reservation-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'restaurant_reservation_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Reservation Norms';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/restaurant-reservation-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'restaurant_reservation_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'restaurant-reservation-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-restaurantreservation-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `restaurant-reservation-checker-${c.slug}.html`);

// Generates drinking-age-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-drinkingage-page.js scripts/data-drinkingage-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  const isProhibited = c.minAge === 0;
  const ageLabel = isProhibited ? 'illegal' : `age ${c.minAge}`;
  const faqs = [
    { q: `What's the legal drinking age in ${c.name}?`, a: isProhibited ? `Alcohol is entirely illegal in ${c.name}, regardless of age. ${c.note}` : `The legal drinking age in ${c.name} is ${c.minAge}. ${c.note}` },
    { q: `Will I be asked for ID in ${c.name}?`, a: isProhibited ? `Not applicable — alcohol is not legally available in ${c.name}.` : `Yes, especially if you're close to the minimum age of ${c.minAge} — carry a valid passport, since a foreign driver's license isn't always accepted as proof of age.` },
    { q: `Is the drinking age the same everywhere in ${c.name}?`, a: `Rules can vary between regions/states within a country — this reflects the national or most common standard for ${c.name}, so check locally if you're near the threshold.` },
    { q: `Is this different from whether ${c.name} is a "dry" country?`, a: `Yes — this is specifically about the minimum legal age, not general alcohol availability or dry regions, which is a separate consideration.` },
  ];
...
<head>
    ...
    <title>Legal Drinking Age Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="What's the legal drinking age in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/drinking-age-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${ageLabel}</span>
        <h1>Legal Drinking Age Checker for ${c.name}</h1>
        <p>The legal drinking age in ${c.name} is ${ageLabel}.</p>
    </div>
...
${countryOptions}
                ...
                <input type="number" id="age" min="1" max="120" placeholder="e.g. 20">

                <div id="calc-alert" class="alert alert-error"></div>
                <button class="btn" id="calc-btn" onclick="calculate()">Check Drinking Age</button>
            </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="drinking-age-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            if (!age) {
                alertEl.textContent = 'Please enter your age.';
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/drinking-age-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'drinking_age_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.status === 'under' || data.result.status === 'prohibited' ? ' warn' : data.result.status === 'just-over' ? ' caution' : '');
                ...
                btn.disabled = false;
                btn.textContent = 'Check Drinking Age';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/drinking-age-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'drinking_age_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'drinking-age-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-drinkingage-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `drinking-age-checker-${c.slug}.html`);

// Generates smoking-vaping-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-smokingvaping-page.js scripts/data-smokingvaping-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  const smokingLabel = SMOKING_LABELS[c.smoking];
  const vapingLabel = VAPING_LABELS[c.vaping];
  const headline = `In ${c.name}, vaping is ${vapingLabel}. Smoking rules are ${smokingLabel}.`;

  const faqs = [
    { q: `Is vaping legal in ${c.name}?`, a: `In ${c.name}, vaping is ${vapingLabel}. ${c.note}` },
    { q: `Can I smoke indoors in ${c.name}?`, a: `Smoking rules in ${c.name} are ${smokingLabel}.` },
    { q: `Can I bring my own vape into ${c.name}?`, a: c.vaping === 'banned' ? `No — bringing an e-cigarette into ${c.name} risks confiscation, fines, or prosecution. Leave it at home.` : c.vaping === 'restricted' ? `You can generally bring a personal device, but bulk quantities or unusual flavors are more likely to draw customs attention in ${c.name}.` : `Yes — vaping is legal in ${c.name}, though check your airline's rules on carrying batteries and e-liquid in the cabin.` },
    { q: `Do these rules apply nationwide in ${c.name}?`, a: `This reflects the general national pattern for ${c.name} — enforcement can vary by city or region, and rules can change quickly, so check current status close to your travel dates.` },
  ];
...
<head>
    ...
    <title>Smoking & Vaping Legality Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="Is vaping legal in ${c.name}? Free instant checker for smoking rules and e-cigarette legality, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/smoking-vaping-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Smoking & Vaping Legality Checker for ${c.name}">
    <meta property="og:description" content="Instant free checker: ${headline}">
    <meta property="og:url" content="https://travelsmarterapp.com/smoking-vaping-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/smoking-vaping-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/smoking-vaping-checker.png">
    <meta name="twitter:title" content="Smoking & Vaping Legality Checker for ${c.name}">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · vaping ${c.vaping}</span>
        <h1>Smoking & Vaping Legality Checker for ${c.name}</h1>
        <p>${headline}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Smoking & Vaping Rules</button>
            </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="smoking-vaping-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/smoking-vaping-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'smoking_vaping_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.vaping === 'banned' ? ' warn' : data.result.vaping === 'restricted' ? ' caution' : '');
                ...
                btn.disabled = false;
                btn.textContent = 'Check Smoking & Vaping Rules';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/smoking-vaping-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'smoking_vaping_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'smoking-vaping-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-smokingvaping-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `smoking-vaping-checker-${c.slug}.html`);

// Generates best-time-to-book-flights-to-{slug}.html pages from structured
// destination data, following the exact template established by the
// hand-built Paris/London pages. Run: node scripts/generate-destination-page.js
const fs = require('fs');
const path = require('path');

const TRIP_TYPE_LABELS = {
  long_haul_transatlantic: 'Long-haul transatlantic',
  long_haul_asia_pacific: 'Long-haul international',
  short_haul: 'Short-haul international',
};

const WINDOW_TEXT = {
  long_haul_transatlantic: { budget: '3 to 5 weeks', premium: '4 to 6 weeks' },
  long_haul_asia_pacific: { budget: '5 to 7 weeks', premium: '6 to 8 weeks' },
  short_haul: { budget: '2 to 3 weeks', premium: '3 to 5 weeks' },
};
...
function render(d) {
  const routeLabel = TRIP_TYPE_LABELS[d.tripType];
  const cityAirport = `${d.city} (${d.airportCode})`;
  const faqHtml = d.faqs.map(f => `            <div class="faq-item">
                <h3>${f.q}</h3>
                <p>${f.a}</p>
            </div>`).join('\n');
...
<head>
    ...
    <title>Best Time to Book Flights to ${cityAirport} From the US | TravelSmarter</title>
    <meta name="description" content="${d.metaDescription}">
    <link rel="canonical" href="https://travelsmarterapp.com/best-time-to-book-flights-to-${d.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Best Time to Book Flights to ${cityAirport} From the US">
    <meta property="og:description" content="Free calculator: find the exact optimal window to book your flight to ${d.city} for the lowest fare.">
    <meta property="og:url" content="https://travelsmarterapp.com/best-time-to-book-flights-to-${d.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/best-time-to-book-flights.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/best-time-to-book-flights.png">
    <meta name="twitter:title" content="Best Time to Book Flights to ${cityAirport} From the US">
    <meta name="twitter:description" content="Free calculator: find the exact optimal window to book your flight to ${d.city} for the lowest fare.">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">USA → ${cityAirport} · ${routeLabel}</span>
        <h1>Best Time to Book Flights to ${d.city}</h1>
        <p>${d.heroText}</p>
    </div>
    ...
    <div class="container">
        <div class="card">
            <div id="form-section">
                <label for="airlineType">Airline type</label>
                <select id="airlineType">
                    <option value="budget">${d.budgetLabel}</option>
                    <option value="premium" selected>${d.premiumLabel}</option>
                </select>
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Calculate Best Time to Book</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full report?</strong> Get a detailed PDF with the reasoning behind your window, plus ${d.city}-specific timing tips — free.</p>
        ...
        <div class="card content-block">
            <h2>Flying to ${d.city} from the US</h2>
            <p>${d.flyingParagraph1}</p>
            <p>${d.flyingParagraph2}</p>
            <h2>Budget vs. premium carriers on this route</h2>
            <p>${d.budgetVsPremiumParagraph}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="best-time-to-book-flights.html">Try the general Best Time to Book calculator →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            if (!departureDate) {
                alertEl.textContent = 'Please choose a departure date.';
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/best-time-to-book/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ tripType: TRIP_TYPE, airlineType, departureDate }),
                });
                const data = await res.json();
                if (!data.success) throw new Error(data.error || 'Something went wrong');

                lastResult = data.result;
                gtag('event', 'tool_calculate', { tool: 'best_time_to_book_flights_to_${d.slug}' });

                document.getElementById('result-headline').textContent = data.result.headline;
                document.getElementById('result-meta').textContent =
                    \`${cityAirport} · \${data.result.airlineTypeLabel} · optimal window: \${data.result.windowOpensAtLabel} – \${data.result.windowClosesAtLabel}\`;
                ...
                btn.disabled = false;
                btn.textContent = 'Calculate Best Time to Book';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/best-time-to-book/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'best_time_to_book_flights_to_${d.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'best-time-to-book-flights-to-${d.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-destination-page.js <data-file.js>');
  process.exit(1);
}
...
destinations.forEach(d => {
  const html = render(d);
  const outPath = path.join(outDir, `best-time-to-book-flights-to-${d.slug}.html`);

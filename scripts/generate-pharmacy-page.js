// Generates pharmacy-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-pharmacy-page.js scripts/data-pharmacy-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = PHARMACY_LABELS[c.pharmacyLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const pharmacyLabel = PHARMACY_LABELS[c.pharmacyLevel];

  const faqs = [
    { q: `Can I buy medication at a pharmacy in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${pharmacyLabel}" mean?`, a: c.pharmacyLevel === 'easy-otc' ? `In ${c.name}, pharmacies are widespread and notably lenient — many medications that require a prescription elsewhere are commonly sold over-the-counter.` : c.pharmacyLevel === 'standard' ? `In ${c.name}, pharmacies are widespread and reliable, with prescription rules broadly similar to most Western countries.` : c.pharmacyLevel === 'limited-access' ? `In ${c.name}, pharmacies are reliable mainly in cities, with real gaps in stock or English-language help elsewhere.` : `In ${c.name}, pharmacy access is genuinely unreliable — don't count on finding what you need locally.` },
    { q: `Should I bring my own medication instead of buying it there?`, a: "Yes, for anything you can't risk running out of — bring the full amount for your trip plus several extra days, along with a doctor's letter, rather than planning to restock locally, even in easy-access destinations." },
...
<head>
    ...
    <title>${c.name} Pharmacy & OTC Medication Checker | TravelSmarter</title>
    <meta name="description" content="Can you buy medication at a pharmacy in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/pharmacy-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Pharmacy & OTC Medication Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/pharmacy-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/pharmacy-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/pharmacy-checker.png">
    <meta name="twitter:title" content="${c.name} Pharmacy & OTC Medication Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${pharmacyLabel}</span>
        <h1>${c.name} Pharmacy & OTC Medication Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Pharmacy Access</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with pharmacy tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${pharmacyLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="pharmacy-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/pharmacy-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'pharmacy_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Pharmacy Access';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/pharmacy-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'pharmacy_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'pharmacy-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-pharmacy-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `pharmacy-checker-${c.slug}.html`);

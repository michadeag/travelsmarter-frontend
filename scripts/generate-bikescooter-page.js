// Generates bike-scooter-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-bikescooter-page.js scripts/data-bikescooter-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Can I rent a bike or e-scooter in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${availabilityLabel}" mean?`, a: c.availabilityLevel === 'widespread' ? `In ${c.name}, app-based bike and/or e-scooter rental is well-established and genuinely easy to use as a tourist, typically with just a foreign credit card.` : c.availabilityLevel === 'limited' ? `In ${c.name}, bike/scooter rental is real but patchy — available in the main city center or tourist area, but coverage thins out or disappears elsewhere.` : c.availabilityLevel === 'rare' ? `In ${c.name}, there's little to no organized bike/scooter-share system — traditional rental shops are the practical option instead.` : `In ${c.name}, e-scooters specifically face a notable legal restriction on top of otherwise limited availability.` },
    { q: `What do I need to use a bike/scooter rental app?`, a: 'Most apps require a credit card and sometimes a local phone number for SMS verification — search the App Store/Play Store for "[city name] bike share" or "[city name] scooter" shortly before your trip, since operators change too fast for any static list to stay current.' },
...
<head>
    ...
    <title>${c.name} Bike & E-Scooter Rental Checker | TravelSmarter</title>
    <meta name="description" content="Can you rent a bike or e-scooter in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/bike-scooter-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Bike & E-Scooter Rental Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Bike & Scooter Availability</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with getting-around tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="bike-scooter-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/bike-scooter-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'bike_scooter_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Bike & Scooter Availability';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/bike-scooter-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'bike_scooter_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'bike-scooter-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-bikescooter-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `bike-scooter-checker-${c.slug}.html`);

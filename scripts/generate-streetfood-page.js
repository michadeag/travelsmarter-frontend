// Generates street-food-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-streetfood-page.js scripts/data-streetfood-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Is it safe to eat street food in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${riskLabel}" mean?`, a: c.riskLevel === 'low' ? `${c.name}'s street food comes with minimal caution needed thanks to high hygiene standards.` : c.riskLevel === 'moderate' ? `${c.name} has a real, well-known street food culture that's generally safe from busy stalls, but a bit of caution pays off.` : `${c.name} has a well-documented, elevated risk of food-borne illness from street food — extra care is warranted, especially for sensitive stomachs.` },
    { q: `What's the single best rule for eating street food safely?`, a: "Eat where there's a visible line of locals. High turnover means fresher ingredients and less time for bacteria to grow — it's the single best predictor of safety almost anywhere in the world." },
...
<head>
    ...
    <title>${c.name} Street Food Safety Checker | TravelSmarter</title>
    <meta name="description" content="Is it safe to eat street food in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/street-food-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Street Food Safety Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/street-food-checker-${c.slug}.html">
    <meta property="og:image" content="https://travelsmarterapp.com/og-images/street-food-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://travelsmarterapp.com/og-images/street-food-checker.png">
    <meta name="twitter:title" content="${c.name} Street Food Safety Checker">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Street Food Safety Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Street Food Safety</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with street food safety tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="street-food-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/street-food-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'street_food_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Street Food Safety';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/street-food-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'street_food_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'street-food-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-streetfood-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `street-food-checker-${c.slug}.html`);

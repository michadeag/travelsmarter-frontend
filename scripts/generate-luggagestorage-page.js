// Generates luggage-storage-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-luggagestorage-page.js scripts/data-luggagestorage-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Where can I store my luggage in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${availabilityLabel}" mean?`, a: c.availabilityLevel === 'excellent' ? `${c.name} has extensive official station/airport lockers plus strong app-based coverage — this is genuinely easy to solve almost anywhere.` : c.availabilityLevel === 'good' ? `${c.name} has real, readily available storage options in major cities, though less extensive than the top tier.` : c.availabilityLevel === 'limited' ? `${c.name} has storage options findable in the capital or main tourist areas, but coverage is patchy elsewhere.` : `Dedicated storage services are essentially nonexistent in ${c.name} — hotel storage before/after your stay is the reliable option.` },
    { q: `What are app-based storage services?`, a: 'Apps like Bounce, Nannybag, Stasher, and LuggageHero let you book storage at partner shops, hotels, and cafes — check their coverage map for your specific city, since availability varies a lot by app and location.' },
    { q: `What if there\'s no dedicated storage service nearby?`, a: 'Most hotels will store bags for free before check-in or after checkout, even if you\'re not staying there that specific night — it\'s always worth asking, even at a hotel you\'re not booked into.' },
  ];
...
<head>
    ...
    <title>${c.name} Luggage Storage Guide | TravelSmarter</title>
    <meta name="description" content="Where can you store your luggage in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/luggage-storage-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Luggage Storage Guide">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/luggage-storage-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/luggage-storage-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/luggage-storage-checker.png">
    <meta name="twitter:title" content="${c.name} Luggage Storage Guide">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Luggage Storage Guide</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Luggage Storage Options</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with luggage storage tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="luggage-storage-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/luggage-storage-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'luggage_storage_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Luggage Storage Options';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/luggage-storage-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'luggage_storage_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'luggage-storage-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-luggagestorage-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `luggage-storage-checker-${c.slug}.html`);

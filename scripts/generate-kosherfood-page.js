// Generates kosher-food-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-kosherfood-page.js scripts/data-kosherfood-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = AVAILABILITY_LABELS[c.availabilityLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const availabilityLabel = AVAILABILITY_LABELS[c.availabilityLevel];

  const faqs = [
    { q: `Is kosher food easy to find in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${availabilityLabel}" mean?`, a: c.availabilityLevel === 'widespread' ? `In ${c.name}, kosher is the mainstream default — you generally don't need to search it out specifically.` : c.availabilityLevel === 'good' ? `${c.name} has a large, well-established Jewish community, so kosher restaurants and shops are genuinely easy to find, especially in major cities.` : c.availabilityLevel === 'limited' ? `Kosher food exists in ${c.name} but requires a bit of research — often via a local synagogue, kosher deli, or Chabad House — and options thin out quickly outside major cities.` : `Dedicated kosher food is genuinely hard to find in ${c.name} — plan ahead, research specific establishments, or be ready to self-cater.` },
    { q: `What's a Chabad House, and can it help here?`, a: "Chabad Houses are Jewish community centers that operate in a surprising number of cities worldwide, and many welcome travelers for a kosher meal — check chabad.org for the nearest location before you go, especially useful in destinations with otherwise limited kosher options." },
    { q: `What can I do if kosher food is hard to find?`, a: "Pre-packaged kosher-certified goods (look for a hechsher symbol) are increasingly available at larger international supermarkets, and self-catering with a kitchenette is a reliable backup in destinations with very limited options." },
  ];
...
<head>
    ...
    <title>${c.name} Kosher Food Availability | TravelSmarter</title>
    <meta name="description" content="Is kosher food easy to find in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/kosher-food-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Kosher Food Availability">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/kosher-food-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/kosher-food-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/kosher-food-checker.png">
    <meta name="twitter:title" content="${c.name} Kosher Food Availability">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${availabilityLabel}</span>
        <h1>${c.name} Kosher Food Availability</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Kosher Food Availability</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with kosher food tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${availabilityLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="kosher-food-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/kosher-food-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'kosher_food_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Kosher Food Availability';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/kosher-food-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'kosher_food_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'kosher-food-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-kosherfood-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `kosher-food-checker-${c.slug}.html`);

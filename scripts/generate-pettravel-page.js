// Generates pet-travel-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-pettravel-page.js scripts/data-pettravel-countries.js
const fs = require('fs');
...
function headline(c) {
  return `Bringing a pet to ${c.name}: ${STATUS_LABELS[c.status]}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const faqs = [
    { q: `Can I bring my dog or cat to ${c.name}?`, a: `Bringing a pet to ${c.name} is ${STATUS_LABELS[c.status]}. ${c.note}` },
    { q: `Will my pet need to go into quarantine in ${c.name}?`, a: c.status === 'strict' ? `Quarantine or a mandatory waiting period is a real possibility — check current requirements well ahead of your trip.` : `Quarantine generally isn't required for a properly prepared pet, but always confirm current rules before you travel.` },
    { q: `How far in advance should I start the paperwork for ${c.name}?`, a: c.status === 'strict' ? `Months in advance — blood titer tests and quarantine arrangements often have mandatory waiting periods that can't be rushed at any price.` : `A few weeks in advance is usually enough, but your pet's health certificate is typically only valid for a short window before departure, so timing still matters.` },
    { q: `Does my airline have its own pet policy for flights to ${c.name}?`, a: `Yes — check your airline's cabin/cargo pet policy separately from ${c.name}'s import rules, since carrier size limits and breed restrictions vary by airline regardless of the destination country's requirements.` },
  ];
...
<head>
    ...
    <title>Bringing a Pet to ${c.name} | TravelSmarter</title>
    <meta name="description" content="Can you bring your pet to ${c.name}? Free instant checker with rabies, quarantine, and import permit rules, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/pet-travel-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Bringing a Pet to ${c.name}">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/pet-travel-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/pet-travel-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/pet-travel-checker.png">
    <meta name="twitter:title" content="Bringing a Pet to ${c.name}">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${STATUS_LABELS[c.status]}</span>
        <h1>Bringing a Pet to ${c.name}</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Pet Travel Rules</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with tips for traveling with your pet to ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="pet-travel-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/pet-travel-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'pet_travel_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Pet Travel Rules';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/pet-travel-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'pet_travel_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'pet-travel-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-pettravel-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `pet-travel-checker-${c.slug}.html`);

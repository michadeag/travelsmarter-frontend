// Generates drone-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-drone-page.js scripts/data-drone-countries.js
const fs = require('fs');
...
function headline(c) {
  return `${c.name}: drones are ${STATUS_LABELS[c.status]}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const faqs = [
    { q: `Can I fly my drone in ${c.name}?`, a: `Drones in ${c.name} are ${STATUS_LABELS[c.status]}. ${c.note}` },
    { q: `Do I need to register my drone before visiting ${c.name}?`, a: c.status === 'banned' ? `Registration exists on paper, but tourist drone use is effectively banned in practice — consider leaving your drone at home.` : `Yes — register before you fly, not after you land. Few authorities offer same-day approval, so this needs to happen before your trip.` },
    { q: `Will my drone be confiscated at customs in ${c.name}?`, a: c.status === 'banned' ? `It's a real risk — drones are commonly confiscated at customs in ${c.name} for tourists without the right paperwork.` : `Unlikely if you've completed the required registration, but keep your registration certificate or permit accessible in case customs asks.` },
    { q: `Are there photography restrictions in ${c.name} even without a drone?`, a: `Yes — photographing military sites, government buildings, and some infrastructure can be restricted almost anywhere in the world, drone or no drone. When in doubt, don't photograph it.` },
  ];
...
<head>
    ...
    <title>Drone Laws in ${c.name} | TravelSmarter</title>
    <meta name="description" content="Can you fly a drone in ${c.name}? Free instant checker with registration rules and photography restrictions, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/drone-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Drone Laws in ${c.name}">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/drone-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/drone-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/drone-checker.png">
    <meta name="twitter:title" content="Drone Laws in ${c.name}">
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Drone Rules</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with tips for flying in ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="drone-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/drone-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'drone_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Drone Rules';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/drone-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'drone_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'drone-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-drone-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `drone-checker-${c.slug}.html`);

// Generates souvenir-export-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-souvenirexport-page.js scripts/data-souvenirexport-countries.js
const fs = require('fs');
...
function headline(c) {
  ...
  return `${c.name}: ${label} — watch out for ${c.topRisk.toLowerCase()}.`;
}
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Are there souvenirs I shouldn't buy or bring home from ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${riskLabel}" mean?`, a: c.riskLevel === 'low' ? `No particularly restricted souvenir category stands out for this destination — the usual global rules still apply (no ivory, no protected wildlife products, no undeclared antiquities).` : c.riskLevel === 'moderate' ? `A specific category of souvenir here carries real export restrictions or requires documentation — check the note above before you buy.` : `Meaningful legal risk exists — confiscation, fines, or even criminal charges are possible for the category noted above. Get an export permit or receipt, or skip it.` },
    { q: `Will customs actually check this when I fly home?`, a: `Enforcement varies, but wildlife products (CITES-protected items), cultural antiquities, and anything requiring a permit are exactly what customs and airport security are trained to look for — both on departure and on arrival.` },
    { q: `Does this apply to every souvenir shop in ${c.name}?`, a: `Reputable shops selling genuinely restricted items (like real ivory or protected coral) are rare and often illegal themselves — but "as seen in the market" is not proof an item is legal to export. When in doubt, ask for documentation or skip it.` },
  ];
...
<head>
    ...
    <title>${c.name} Souvenir & Export Rules | TravelSmarter</title>
    <meta name="description" content="What souvenirs are risky or restricted to bring home from ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/souvenir-export-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Souvenir & Export Rules</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Souvenir Rules</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with souvenir & export tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${riskLabel}: ${c.topRisk}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="souvenir-export-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/souvenir-export-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'souvenir_export_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Souvenir Rules';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/souvenir-export-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'souvenir_export_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'souvenir-export-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-souvenirexport-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `souvenir-export-checker-${c.slug}.html`);

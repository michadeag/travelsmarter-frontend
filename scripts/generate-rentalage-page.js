// Generates rental-age-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-rentalage-page.js scripts/data-rentalage-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  const faqs = [
    { q: `What's the minimum age to rent a car in ${c.name}?`, a: `The minimum rental age in ${c.name} is generally ${c.minAge}. ${c.note}` },
    { q: `Is there a young-driver surcharge in ${c.name}?`, a: `Drivers under ${c.youngThreshold} typically pay extra in ${c.name}. ${c.feeNote}` },
    { q: `Does the young-driver fee in ${c.name} show up in the online quote?`, a: `Not always — young-driver surcharges are frequently added at the rental counter rather than shown during online booking, so it's worth asking directly or calling ahead if you're near the age threshold.` },
    { q: `What else should I check before renting a car in ${c.name}?`, a: `Confirm how long you need to have held your license, whether an International Driving Permit is required alongside it, and whether the credit card used for the deposit must match the primary driver's name exactly — all common extra requirements beyond age alone.` },
  ];
...
<head>
    ...
    <title>Car Rental Age Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="What's the minimum car rental age in ${c.name}, and will you pay a young-driver fee? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/rental-age-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · Min. age ${c.minAge}</span>
        <h1>Car Rental Age Checker for ${c.name}</h1>
        <p>Young-driver fee applies under ${c.youngThreshold}. ${c.feeNote}</p>
    </div>
...
${countryOptions}
                ...
                <input type="number" id="age" min="1" max="120" placeholder="e.g. 23">

                <div id="calc-alert" class="alert alert-error"></div>
                <button class="btn" id="calc-btn" onclick="calculate()">Check Rental Age Rules</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with rental tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="rental-age-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            if (!age) {
                alertEl.textContent = 'Please enter the driver\\'s age.';
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/rental-age-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ country, age }),
                });
                const data = await res.json();
                if (!data.success) throw new Error(data.error || 'Something went wrong');

                lastResult = data.result;
                gtag('event', 'tool_calculate', { tool: 'rental_age_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.status === 'too-young' ? ' warn' : data.result.status === 'young-fee' ? ' caution' : '');
                document.getElementById('result-meta').textContent = data.result.feeNote + ' ' + data.result.note;
                ...
                btn.disabled = false;
                btn.textContent = 'Check Rental Age Rules';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/rental-age-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'rental_age_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'rental-age-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-rentalage-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `rental-age-checker-${c.slug}.html`);

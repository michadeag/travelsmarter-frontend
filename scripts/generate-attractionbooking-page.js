// Generates attraction-booking-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-attractionbooking-page.js scripts/data-attractionbooking-countries.js
const fs = require('fs');
...
function headline(c) {
  const label = BOOKING_LABELS[c.bookingLevel];
  return `${c.name}: ${label}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const bookingLabel = BOOKING_LABELS[c.bookingLevel];

  const faqs = [
    { q: `Do I need to book attractions in advance for ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${bookingLabel}" mean?`, a: c.bookingLevel === 'essential' ? `In ${c.name}, marquee attractions require advance booking or permits, sometimes weeks or months ahead — plan before you land, not after.` : c.bookingLevel === 'recommended' ? `In ${c.name}, walk-up often works, but advance booking avoids real lines or sold-out situations at popular sites.` : c.bookingLevel === 'flexible' ? `In ${c.name}, there's a genuine mix — a few marquee sites need booking, but most things are walk-up.` : `In ${c.name}, most attractions are walk-up friendly, and advance booking is rarely needed.` },
    { q: `What if a specific landmark matters most to my trip?`, a: "Check that attraction's own official booking page directly as soon as you've fixed your travel dates — the most famous sites can sell out weeks or months ahead in peak season, regardless of the destination's general pattern." },
...
<head>
    ...
    <title>${c.name} Attraction Booking Checker | TravelSmarter</title>
    <meta name="description" content="Do you need to book attractions in advance for ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/attraction-booking-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Attraction Booking Checker">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/attraction-booking-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/attraction-booking-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/attraction-booking-checker.png">
    <meta name="twitter:title" content="${c.name} Attraction Booking Checker">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${bookingLabel}</span>
        <h1>${c.name} Attraction Booking Checker</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Attraction Booking Norms</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with sightseeing tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${bookingLabel}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="attraction-booking-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/attraction-booking-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'attraction_booking_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Attraction Booking Norms';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/attraction-booking-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'attraction_booking_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'attraction-booking-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-attractionbooking-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `attraction-booking-checker-${c.slug}.html`);

// Generates public-restroom-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-publicrestroom-page.js scripts/data-publicrestroom-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `What should I expect from public restrooms in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${availabilityLabel}" mean?`, a: c.availabilityLevel === 'easy' ? `${c.name} has plentiful, familiar Western-style restrooms with few surprises.` : c.availabilityLevel === 'moderate' ? `${c.name} has some genuine etiquette differences worth knowing — check the note above so nothing catches you off guard.` : `${c.name} requires real adjustment — squat toilets and carrying your own tissue are genuinely common outside hotels and malls.` },
    { q: `Should I carry my own tissues?`, a: "It's a good habit almost everywhere, and genuinely essential in destinations marked \"tricky\" here — paper isn't always provided, especially outside hotels, malls, and major tourist sites." },
    { q: `What if I see a small bin next to the toilet?`, a: "That's usually a sign the local plumbing can't handle paper — put used toilet paper in the bin instead of flushing it. This is a real, common norm in much of Latin America and parts of southern/eastern Europe, not a hygiene downgrade." },
  ];
...
<head>
    ...
    <title>${c.name} Public Restroom Guide | TravelSmarter</title>
    <meta name="description" content="What should you expect from public restrooms in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/public-restroom-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Public Restroom Guide">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/public-restroom-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/public-restroom-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/public-restroom-checker.png">
    <meta name="twitter:title" content="${c.name} Public Restroom Guide">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Public Restroom Guide</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Public Restroom Guide</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with public restroom tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="public-restroom-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/public-restroom-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'public_restroom_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Public Restroom Guide';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/public-restroom-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'public_restroom_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'public-restroom-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-publicrestroom-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `public-restroom-checker-${c.slug}.html`);

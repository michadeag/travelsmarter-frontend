// Generates vat-refund-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-vatrefund-page.js scripts/data-vatrefund-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  const refundLabel = REFUND_LABELS[c.refundAvailable];
  const headline = c.refundAvailable === 'yes'
    ? `${c.name}'s VAT/GST refund for tourists is ${refundLabel}. Standard rate: ${c.vatRate}%. Minimum spend: ${c.minimumSpend}.`
    : `${c.name}'s VAT/GST refund for tourists is ${refundLabel}. Standard rate: ${c.vatRate}%.`;

  const faqs = [
    { q: `Can I get a VAT/tax refund for shopping in ${c.name}?`, a: `${c.name}'s VAT/GST refund for tourists is ${refundLabel} ${c.note}` },
    { q: `What's the standard VAT/GST rate in ${c.name}?`, a: `The standard rate in ${c.name} is ${c.vatRate}%.` },
    { q: c.refundAvailable === 'yes' ? `What's the minimum spend to qualify for a refund in ${c.name}?` : `Is there any way to get a refund in ${c.name}?`, a: c.refundAvailable === 'yes' ? `The minimum spend to qualify is ${c.minimumSpend}.` : `No general tourist refund scheme exists — duty-free shops after airport security remain a separate, unrelated system.` },
    { q: `Do I need to do anything before I fly to claim a refund in ${c.name}?`, a: c.refundAvailable === 'yes' ? `Yes — keep receipts and purchased goods accessible for inspection, and validate your refund forms at customs or an electronic kiosk before checking in your luggage.` : `No refund paperwork is needed since no general scheme exists here.` },
  ];
...
<head>
    ...
    <title>VAT/Tax-Free Shopping Refund Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="Can you get a VAT/tax refund for shopping in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/vat-refund-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="VAT/Tax-Free Shopping Refund Checker for ${c.name}">
    <meta property="og:description" content="Instant free checker: ${headline}">
    <meta property="og:url" content="https://travelsmarterapp.com/vat-refund-checker-${c.slug}.html">
    <meta property="og:image" content="https://api.travelsmarterapp.com/og-images/vat-refund-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://api.travelsmarterapp.com/og-images/vat-refund-checker.png">
    <meta name="twitter:title" content="VAT/Tax-Free Shopping Refund Checker for ${c.name}">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · refund ${c.refundAvailable}</span>
        <h1>VAT/Tax-Free Shopping Refund Checker for ${c.name}</h1>
        <p>${headline}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Tax Refund Rules</button>
            </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="vat-refund-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/vat-refund-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'vat_refund_checker_${c.slug}' });

                const headlineEl = document.getElementById('result-headline');
                headlineEl.textContent = data.result.headline;
                headlineEl.className = 'result-headline' + (data.result.refundAvailable === 'no' ? ' warn' : '');
                ...
                btn.disabled = false;
                btn.textContent = 'Check Tax Refund Rules';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/vat-refund-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'vat_refund_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'vat-refund-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-vatrefund-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `vat-refund-checker-${c.slug}.html`);

// Generates tourist-scams-checker-{slug}.html pages, one per
...
// Run: node scripts/generate-touristscams-page.js scripts/data-touristscams-countries.js
const fs = require('fs');
...
function headline(c) {
  ...
  return `${c.name}: ${label} — watch out for ${c.topScam.toLowerCase()}.`;
}
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `What's the most common tourist scam in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${riskLabel}" mean?`, a: c.riskLevel === 'low' ? `No widespread, well-documented scam pattern stands out for this destination beyond generic pickpocketing — a genuinely low-risk destination for tourist scams.` : c.riskLevel === 'moderate' ? `A real, somewhat common scam pattern exists here — check the note above so you can recognize it before it happens to you.` : `A well-documented, actively-run scam catches tourists here regularly — read the note above carefully so you know exactly what to watch for.` },
    { q: `How do I avoid getting scammed in ${c.name}?`, a: `Agree on prices before getting into a taxi, be skeptical of unusually friendly strangers steering you somewhere specific, and never sign anything or accept food/drinks from someone you just met. The single biggest risk for this destination is noted above — general street-smarts cover the rest.` },
    { q: `Does this mean ${c.name} isn't safe to visit?`, a: `Not at all — millions of tourists visit safely every year. Knowing the one scam pattern that catches people most often is simply the best, cheapest defense there is, since it's designed specifically to look harmless until it's too late.` },
  ];
...
<head>
    ...
    <title>${c.name} Tourist Scams to Avoid | TravelSmarter</title>
    <meta name="description" content="What's the most common tourist scam in ${c.name}, and how do you avoid it? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/tourist-scams-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Tourist Scams to Avoid">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/tourist-scams-checker-${c.slug}.html">
    <meta property="og:image" content="https://travelsmarterapp.com/og-images/tourist-scams-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://travelsmarterapp.com/og-images/tourist-scams-checker.png">
    <meta name="twitter:title" content="${c.name} Tourist Scams to Avoid">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Tourist Scams to Avoid</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Common Scams</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with scam-avoidance tips for ${c.name} — free.</p>
        ...
        <div class="card content-block">
            ...
            <p><strong>${riskLabel}: ${c.topScam}.</strong> ${c.note}</p>
        </div>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="tourist-scams-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/tourist-scams-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'tourist_scams_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Common Scams';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/tourist-scams-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'tourist_scams_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'tourist-scams-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-touristscams-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `tourist-scams-checker-${c.slug}.html`);

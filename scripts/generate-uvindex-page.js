// Generates uv-index-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-uvindex-page.js scripts/data-uvindex-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const spf = SPF_RECOMMENDATIONS[c.level];
  const faqs = [
    { q: `How intense is the UV in ${c.name}?`, a: `${c.name} has ${label}. ${c.note}` },
    { q: `What SPF should I pack for ${c.name}?`, a: spf },
    { q: `Can I get sunburned in ${c.name} on a cloudy day?`, a: `Yes — up to 80% of UV rays pass through cloud cover, so overcast skies in ${c.name} don't mean you can skip sun protection.` },
    { q: `Does altitude matter for sun exposure in ${c.name}?`, a: `Yes — UV exposure increases roughly 10% for every 1,000 meters of altitude, so mountain or high-plateau areas within ${c.name} need extra protection compared to sea-level spots at the same latitude.` },
  ];
...
<head>
    ...
    <title>UV Index & Sun Safety Checker for ${c.name} | TravelSmarter</title>
    <meta name="description" content="How intense is the sun in ${c.name}? Free instant checker with SPF recommendations, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/uv-index-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>UV Index & Sun Safety Checker for ${c.name}</h1>
        <p>${c.name} has ${label}.</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check UV & Sun Safety</button>
            </div>

            <div id="result">
                <div class="result-headline" id="result-headline"></div>
                <div class="result-meta" id="result-spf"></div>
                <div class="result-meta" id="result-note"></div>

                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with sun safety tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="uv-index-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/uv-index-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'uv_index_checker_${c.slug}' });

                document.getElementById('result-headline').textContent = data.result.headline;
                document.getElementById('result-spf').textContent = 'Recommended: ' + data.result.spfRecommendation;
                document.getElementById('result-note').textContent = data.result.note;
                ...
                btn.disabled = false;
                btn.textContent = 'Check UV & Sun Safety';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/uv-index-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'uv_index_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'uv-index-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-uvindex-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `uv-index-checker-${c.slug}.html`);

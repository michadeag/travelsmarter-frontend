// Generates currency-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator uses
// so content stays consistent with the live API.
// Run: node scripts/generate-currency-page.js scripts/data-currency-countries.js
const fs = require('fs');
...
function headline(c) {
  return `${c.name} uses the ${c.currencyName} — ${CASH_CULTURE_LABELS[c.cashCulture]}.`;
}
...
function render(c, allCountries) {
  const h = headline(c);
  const faqs = [
    { q: `What currency does ${c.name} use?`, a: `${c.name} uses the ${c.currencyName}.` },
    { q: `Is ${c.name} a cash or card country?`, a: `In ${c.name}, ${CASH_CULTURE_LABELS[c.cashCulture]}. ${c.note}` },
    { q: `Should I exchange currency before I arrive in ${c.name}?`, a: `Exchange rates at airport kiosks are usually the worst available — a local ATM after you land almost always gets you a better rate, and a small amount of cash exchanged in advance is only worth it for late-night arrivals.` },
    { q: `Should I let my card charge me in US dollars or the local currency in ${c.name}?`, a: `Always choose the local currency when a card terminal or ATM in ${c.name} asks — "dynamic currency conversion" into your home currency almost always carries a worse exchange rate.` },
  ];
...
<head>
    ...
    <title>Currency & Cash Culture in ${c.name} | TravelSmarter</title>
    <meta name="description" content="What currency does ${c.name} use, and is it a cash or card country? Free instant checker plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/currency-checker-${c.slug}.html">
...
<body>
    ...
    <div class="hero">
        <span class="route-badge">${c.name} · ${CASH_CULTURE_LABELS[c.cashCulture]}</span>
        <h1>Currency & Cash Culture in ${c.name}</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Currency & Cash Culture</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with cash and card tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="currency-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/currency-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'currency_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Currency & Cash Culture';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/currency-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'currency_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'currency-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-currency-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `currency-checker-${c.slug}.html`);

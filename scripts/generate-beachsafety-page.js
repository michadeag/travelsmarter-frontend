// Generates beach-safety-checker-{slug}.html pages, one per country,
// synthesizing FAQ text from the same fields the backend calculator
// uses so content stays consistent with the live API.
// Run: node scripts/generate-beachsafety-page.js scripts/data-beachsafety-countries.js
const fs = require('fs');
...
function render(c, allCountries) {
  ...
  const faqs = [
    { q: `Is it safe to swim at the beach in ${c.name}?`, a: `${h} ${c.note}` },
    { q: `What does "${riskLabel}" mean?`, a: c.riskLevel === 'low' ? `${c.name}'s beaches are generally calm with minimal serious ocean hazards.` : c.riskLevel === 'moderate' ? `${c.name} has real, if generally manageable, seasonal or local ocean hazards worth knowing about before you swim.` : `${c.name} has well-documented, serious ocean hazards — read the note above carefully and always follow local flag warnings.` },
    { q: `What's the most dangerous ocean hazard, statistically?`, a: "Rip currents — they cause more beach deaths worldwide every year than sharks, jellyfish, and every other ocean hazard combined. If caught in one, don't swim against it; swim parallel to shore until you're free, then head back in at an angle." },
...
<head>
    ...
    <title>${c.name} Beach & Ocean Safety | TravelSmarter</title>
    <meta name="description" content="Is it safe to swim at the beach in ${c.name}? Free instant checker, plus a PDF guide.">
    <link rel="canonical" href="https://travelsmarterapp.com/beach-safety-checker-${c.slug}.html">

    <meta property="og:type" content="website">
    <meta property="og:title" content="${c.name} Beach & Ocean Safety">
    <meta property="og:description" content="Instant free checker: ${h}">
    <meta property="og:url" content="https://travelsmarterapp.com/beach-safety-checker-${c.slug}.html">
    <meta property="og:image" content="https://travelsmarterapp.com/og-images/beach-safety-checker.png">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:image" content="https://travelsmarterapp.com/og-images/beach-safety-checker.png">
    <meta name="twitter:title" content="${c.name} Beach & Ocean Safety">
...
<body>
    ...
    <div class="hero">
        ...
        <h1>${c.name} Beach & Ocean Safety</h1>
        <p>${h}</p>
    </div>
...
${countryOptions}
                ...
                <button class="btn" id="calc-btn" onclick="calculate()">Check Beach & Ocean Safety</button>
            </div>
            ...
            <div id="result">
                ...
                <div id="pdf-section">
                    <p><strong>Want the full guide?</strong> Get a PDF with beach safety tips for ${c.name} — free.</p>
...
${faqHtml}
    ...
    <footer>
        ...
        <a href="beach-safety-checker.html">Check a different country →</a>
    </footer>
    ...
    <script>
        ...
        async function calculate() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/beach-safety-checker/calculate\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                gtag('event', 'tool_calculate', { tool: 'beach_safety_checker_${c.slug}' });
                ...
                btn.disabled = false;
                btn.textContent = 'Check Beach & Ocean Safety';
            }
        }
        ...
        async function downloadPdf() {
            ...
            try {
                const res = await fetch(\`\${API_URL}/api/tools/beach-safety-checker/pdf\`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email, firstName,
                ...
                gtag('event', 'generate_lead', { tool: 'beach_safety_checker_${c.slug}' });

                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'beach-safety-checker-${c.slug}.pdf';
...
if (!dataFile) {
  console.error('Usage: node generate-beachsafety-page.js <data-file.js>');
  process.exit(1);
}
...
countries.forEach(c => {
  ...
  const outPath = path.join(outDir, `beach-safety-checker-${c.slug}.html`);
